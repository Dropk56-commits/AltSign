...
bool crypto_test_x25519_runner(ccdict_t vector)
{
    ...
    for (size_t i = 0; i < CC_ARRAY_LEN(ccec_cp_x25519_impls); i++) {
        ...
        int rc = cccurve25519_internal(cp, out, private->bytes, public->bytes, rng);
...
cleanup:
    if (flags & wycheproof_flag_LowOrderPublic) {
