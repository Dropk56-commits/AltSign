                                   ...
                                   const uint8_t *Z)
{
    ...
    if (Z) {
        ...
        ccdigest_update_internal(di, dc, 32, Z);
        ccdigest_update_internal(di, dc, sizeof(pad), pad);
    }

    ccdigest_update_internal(di, dc, 32, az + 32);
    // Ensure we filled a full input block.
    cc_require_or_return((Z == NULL) || (ccdigest_num(di, dc) == 0), CCERR_INTERNAL);

    ccdigest_update_internal(di, dc, mlen, m);
    ...
    ccdigest_init_internal(di, dc);
    ccdigest_update_internal(di, dc, 32, sig);
    ccdigest_update_internal(di, dc, 32, pk);
    ccdigest_update_internal(di, dc, mlen, m);
...
int cced25519_sign_with_rng_internal(const struct ccdigest_info *di,
                                     struct ccrng_state *rng,
                                     ccec25519signature sig,
                                     size_t len,
                                     const void *msg,
                                     ...
                                     const ccec25519secretkey sk)
{
    ...
    rv = cced25519_make_pub_internal(ccsha512_di(), rng, pk_computed, sk);
    cc_require_or_return(rv == CCERR_OK, rv);
                            ...
                            const ccec25519secretkey sk)
{
    ...
    return cced25519_sign_with_rng_internal(di, rng, sig, len, msg, pk, sk);
}
