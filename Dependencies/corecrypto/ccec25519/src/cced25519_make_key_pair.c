/* Copyright (c) (2014-2019,2021,2022,2024) Apple Inc. All rights reserved.
 *
...
int cced25519_make_pub_internal(const struct ccdigest_info *di, struct ccrng_state *rng, ccec25519pubkey pk, const ccec25519secretkey sk)
{
    cc_require_or_return(di->output_size == 64, CCERR_PARAMETER);

    uint8_t h[64];
    ccdigest_internal(di, sizeof(ccec25519key), sk, h);
    h[0] &= 248;
    h[31] &= 127;
    h[31] |= 64;
