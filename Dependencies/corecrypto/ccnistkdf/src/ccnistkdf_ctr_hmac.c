/* Copyright (c) (2013-2015,2018,2019,2021,2022,2024) Apple Inc. All rights reserved.
 *
...
F (const struct ccdigest_info *di, cchmac_ctx_t hc, const cc_unit *istate,
   size_t counter, size_t fixedDataLen, const void *fixedData, void *result) {
    ...
    ccdigest_update_internal(di, cchmac_digest_ctx(di, hc), fixedDataLen, fixedData);
    cchmac_final_internal(di, hc, result);
}
...
int ccnistkdf_ctr_hmac_fixed(const struct ccdigest_info *di,
    ...
    cchmac_init_internal(di, hc, kdkLen, kdk);
