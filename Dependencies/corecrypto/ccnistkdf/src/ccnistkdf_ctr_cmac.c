/* Copyright (c) (2018,2019,2021,2022,2024) Apple Inc. All rights reserved.
 *
...
PRF_EVAL (struct cccmac_ctx* cmac, uint8_t r, uint32_t counter, size_t fixedData_nbytes, const void *fixedData, void *result)
{
    ...
    cccmac_update_internal(cmac, fixedData_nbytes, fixedData);
    cccmac_final_generate_internal(cmac, cmac->cbc->block_size, result);
}

static int ccnistkdf_ctr_cmac_fixed_internal(const struct ccmode_cbc *cbc,
                                             ...
                                             size_t dk_nbytes, void *dk)
{
    ...
    cccmac_init_internal(cbc, cache, kdk_nbytes, kdk);
...
int ccnistkdf_ctr_cmac_fixed(const struct ccmode_cbc *cbc,
    ...
    return ccnistkdf_ctr_cmac_fixed_internal(cbc, r, kdk_nbytes, kdk, fixedData_nbytes, fixedData, dk_nbytes, dk);
}

int ccnistkdf_ctr_cmac_internal(const struct ccmode_cbc *cbc,
                                uint8_t r, size_t kdk_nbytes, const void *kdk,
                                ...
                                size_t dk_nbytes, size_t dk_len_nbytes, void *dk)
{
    ...
    retval = ccnistkdf_ctr_cmac_fixed_internal(cbc, r, kdk_nbytes, kdk, fixedData_nbytes, fixedData, dk_nbytes, dk);
                       ...
                       size_t dk_nbytes, size_t dk_len_nbytes, void *dk)
{
    ...
    return ccnistkdf_ctr_cmac_internal(cbc, r, kdk_nbytes, kdk, label_nbytes, label, context_nbytes, context, dk_nbytes, dk_len_nbytes, dk);
}
