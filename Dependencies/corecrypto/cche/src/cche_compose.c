    ...
    const uint32_t *cc_counted_by(ctext->npoly *cche_ciphertext_coeff_ctx(ctext)->dims.nmoduli) skip_lsbs)
{
    ...
    return cche_ciphertext_coeff_decompose_nptexts_internal(ctext, skip_lsbs);
}
                                  ...
                                  const uint32_t *cc_counted_by(cche_ciphertext_fresh_npolys() * nmoduli) skip_lsbs)
{
    ...
    cc_require_or_return(nptexts == cche_ciphertext_coeff_decompose_nptexts_internal(ctext, skip_lsbs), CCERR_PARAMETER);
