...
int cche_encode_simd_reduced_int64(cche_plaintext_t ptext,
                                   cche_param_ctx_const_t param_ctx,
                                   uint32_t nvalues,
                                   const int64_t *values)
{
    ...
    ccrns_modulus_const_t t_modulus = ccpolyzp_po2cyc_ctx_ccrns_modulus(cche_param_ctx_plaintext_context(param_ctx), 0);

    const uint32_t *indices = cche_param_ctx_encoding_indices_const(param_ctx);
    for (uint32_t i = 0; i < nvalues; ++i) {
        // Check decoded values are in expected range
        ccrns_int value = ccpolyzp_po2cyc_int_to_rem(values[i], t_modulus);
        cc_require_or_return(value <= (t_modulus->value), CCERR_PARAMETER);
        cc_unit *data = CCPOLYZP_PO2CYC_DATA(poly, 0, indices[i]);
        ccpolyzp_po2cyc_rns_int_to_units(data, value);
    }
    for (uint32_t i = nvalues; i < poly->context->dims.degree; ++i) {
