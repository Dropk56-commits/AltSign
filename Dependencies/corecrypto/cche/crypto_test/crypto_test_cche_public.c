                                                               ...
                                                               cche_predefined_encryption_params_t enc_params)
{
    ...
    // Calculates the number of LSB bits per polynomial that can be excluded
    // from serialization.
    //
    // The LSB bits of each polynomial may be excluded from the serialization,
    // since they are rarely used in BFV decryption. This yields a smaller
    // serialization size, at the cost of a small chance of decryption
    // error. See Section 5.2 of iacr's eprint/2022/207.
    // Note, the noise estimate in Appendix F of this paper assumes
    // a noiseless ciphertext. We change their algorithm to drop bits
    // assuming the ct noise is less than `q/4t`.

    // Given a ciphertext of two polynomials `(b, a)`, the LSBs of the ct can be dropped before decryption.
    // Note, this is a noisy operation so it adds noise to the existing ciphertext.
    // Decryption of a message `m_e` is computed as `round(m_e * t / q)`.

    // Now, if the ct error is less than `q/4t`, then we can
    // have the error added sum to `<= q/4t` and the total error is
    // `< q/4t + q/4t = q/2t` and we can successfully decrypt.
    // The MSB decimal bit of `m_e * t / q` is used
    // to ensure correct rounding, hence why `l' = floor(log2(q / t)) - 2` rather than `floor(log2(q / t))`.
    ...
    // If `b_L + a_L s < 2^{l'}`, we can omit `b_L` and `b_L` from serialization with additional decryption error.

    // Now we derive how many bits to drop:
    ...
    // `z_score * sqrt(2N/9) * 2^{l_a} < 2^{l'-1} < q/8t`, iff
    ...
    // dependence on `b` is deterministic and we can always
    // decrypt assuming that the ciphertext error is less than `q/4t`.
    ...
    uint32_t l_prime = (q >= 2 * t) ? ccpolyzp_po2cyc_log2_uint64(q / t) - 3 : 0;
    expected_nskip_lsbs[0] = l_prime >= 0 ? l_prime : 0;
    ...
    expected_nskip_lsbs[1] = l_prime >= log2_tmp ? l_prime - log2_tmp : 0;

    if (expected_nskip_lsbs[1] <= 1) {
        expected_nskip_lsbs[0] = l_prime + 1 > 0 ? l_prime + 1 : 0;
        expected_nskip_lsbs[1] = 0;
    }
...
static void test_cche_crt_compose(void)
{
    const size_t len = 32;
    uint64_t plaintext_moduli[2] = { 40961, 65537 };
    uint64_t mod = plaintext_moduli[0] * plaintext_moduli[1];
    int64_t values[len];
    int64_t values_submod[len * 2];
    for (uint32_t i = 0; i < CC_ARRAY_LEN(values); ++i) {
        ...
        values_submod[i] = ccpolyzp_po2cyc_rem_to_centered(value % plaintext_moduli[0], plaintext_moduli[0]);
        values_submod[i + len] = ccpolyzp_po2cyc_rem_to_centered(value % plaintext_moduli[1], plaintext_moduli[1]);
    }
    int64_t values_crt_decoded[len];
    int rv = cche_crt_compose(len, values_crt_decoded, values_submod, 2, plaintext_moduli);
    is(rv, CCERR_OK, "cche_crt_compose");
    is(array_eq_int64(CC_ARRAY_LEN(values), values, values_crt_decoded), true, "test_cche_crt_compose");
}
...
int ntests_cche_public(void)
{
    ...
    // Tests run for non of pre-defined parameters
    nall_params_tests += 2; // test_cche_crt_compose
