...
static void test_cche_encode_simd_reduced_int64(cche_scheme_t he_scheme)
{
    CC_DECL_WORKSPACE_TEST(ws);
    ...
    // N values roundtrip
    {
        int64_t values[128] = { 0 };
        for (uint32_t i = 0; i < CC_ARRAY_LEN(values); ++i) {
            values[i] = uniform_int64(1ULL << CCRNS_INT_NBITS_MINUS_1);
        }

        is(cche_encode_simd_reduced_int64(ptext, param_ctx, CC_ARRAY_LEN(values), values),
           CCERR_OK,
           "cche_encode_simd_int64_out_of_bounds != CCERR_OK");
        is(cche_plaintext_modulus(ptext), encrypt_params.plaintext_modulus, "cche_plaintext_modulus");
        ...
        for (uint32_t i = 0; i < CC_ARRAY_LEN(values); ++i) {
            values[i] = ccpolyzp_po2cyc_int_to_centered(
                values[i], ccpolyzp_po2cyc_ctx_ccrns_modulus(cche_param_ctx_plaintext_context(param_ctx), 0));
        }
...
static void test_cche_crt_decode(cche_scheme_t he_scheme)
{
    CC_DECL_WORKSPACE_TEST(ws);

    static struct cche_encrypt_params encrypt_param1 = {
        .poly_modulus_degree = 128, .plaintext_modulus = 40961, .nskip_lsbs = { 0, 0 }, .nmoduli = 1, .moduli = { 114689 }
    };
    static struct cche_encrypt_params encrypt_param2 = {
        .poly_modulus_degree = 128, .plaintext_modulus = 65537, .nskip_lsbs = { 0, 0 }, .nmoduli = 1, .moduli = { 114689 }
    };

    static ccrns_int plaintext_moduli[2] = { 40961, 65537 };
    static ccrns_int plaintext_modulus[1] = { 40961 };

    encrypt_param1.he_scheme = he_scheme;
    encrypt_param2.he_scheme = he_scheme;

    cche_param_ctx_t param_ctx1 = CCHE_PARAM_CTX_ALLOC_WS(ws, &encrypt_param1);
    cche_param_ctx_t param_ctx2 = CCHE_PARAM_CTX_ALLOC_WS(ws, &encrypt_param2);

    CC_TEST_DISABLE_DIT_ASSERTS
    is(cche_param_ctx_init_ws(ws, param_ctx1, &encrypt_param1), CCERR_OK, "BFV/BGV param ctx init");
    is(cche_param_ctx_init_ws(ws, param_ctx2, &encrypt_param2), CCERR_OK, "BFV/BGV param ctx init");
    CC_TEST_ENABLE_DIT_ASSERTS

    ccpolyzp_po2cyc_ctx_const_t plain_ctx1 = cche_param_ctx_plaintext_context(param_ctx1);
    ccpolyzp_po2cyc_ctx_const_t plain_ctx2 = cche_param_ctx_plaintext_context(param_ctx2);

    cche_plaintext_t ptext1 = CCHE_PLAINTEXT_ALLOC_WS(ws, plain_ctx1);
    cche_plaintext_t ptext2 = CCHE_PLAINTEXT_ALLOC_WS(ws, plain_ctx2);

    // N values roundtrip
    {
        static const size_t len = 128;
        uint64_t mod = encrypt_param1.plaintext_modulus * encrypt_param2.plaintext_modulus;
        int64_t values[len];
        for (uint32_t i = 0; i < CC_ARRAY_LEN(values); ++i) {
            uint64_t value = (uint64_t)uniform_int64(1ULL << CCRNS_INT_NBITS_MINUS_1) % mod;
            values[i] = ccpolyzp_po2cyc_rem_to_centered(value, mod);
        }

        is(cche_encode_simd_reduced_int64(ptext1, param_ctx1, CC_ARRAY_LEN(values), values),
           CCERR_OK,
           "cche_encode_simd_reduced_int64 != CCERR_OK");
        is(cche_encode_simd_reduced_int64(ptext2, param_ctx2, CC_ARRAY_LEN(values), values),
           CCERR_OK,
           "cche_encode_simd_reduced_int64 != CCERR_OK");
        is(cche_plaintext_modulus(ptext1), encrypt_param1.plaintext_modulus, "cche_plaintext_modulus");
        is(cche_plaintext_modulus(ptext2), encrypt_param2.plaintext_modulus, "cche_plaintext_modulus");

        int64_t value1[len * 2];
        int64_t *value2 = value1 + len;
        is(cche_decode_simd_int64_ws(ws, param_ctx1, len, value1, ptext1), CCERR_OK, "che_decode_simd_int64_ws != CCERR_OK");
        is(cche_decode_simd_int64_ws(ws, param_ctx2, len, value2, ptext2), CCERR_OK, "che_decode_simd_int64_ws != CCERR_OK");

        struct cche_crt_composer *composer = (struct cche_crt_composer *)CC_ALLOC_WS(ws, struct_cche_crt_composer_nof_n(2));
        CC_TEST_DISABLE_DIT_ASSERTS
        is(cche_crt_composer_init_ws(ws, composer, 2, plaintext_moduli), CCERR_OK, "ccche_crt_composer_init != CCERR_OK");
        CC_TEST_ENABLE_DIT_ASSERTS

        int64_t values_crt_decoded1[len];
        int64_t values_crt_decoded2[len];
        is(cche_crt_composer_compose_ws(ws, composer, len, values_crt_decoded1, value1),
           CCERR_OK,
           "cche_crt_composer_compose != CCERR_OK");
        CC_TEST_DISABLE_DIT_ASSERTS
        is(cche_crt_compose_ws(ws, len, values_crt_decoded2, value1, 2, plaintext_moduli),
           CCERR_OK,
           "cche_crt_compose != CCERR_OK");
        CC_TEST_ENABLE_DIT_ASSERTS

        is(array_eq_int64(CC_ARRAY_LEN(values), values, values_crt_decoded1), true, "test_cche_crt_decode");
        is(array_eq_int64(CC_ARRAY_LEN(values), values, values_crt_decoded2), true, "test_cche_crt_decode");
    }
    ...
    // N values roundtrip, 1 RNS limb
    {
        static const size_t len = 128;
        uint64_t mod = encrypt_param1.plaintext_modulus;
        int64_t values[len];
        for (uint32_t i = 0; i < CC_ARRAY_LEN(values); ++i) {
            uint64_t value = (uint64_t)uniform_int64(1ULL << CCRNS_INT_NBITS_MINUS_1) % mod;
            values[i] = ccpolyzp_po2cyc_rem_to_centered(value, mod);
        ...
        struct cche_crt_composer *composer = (struct cche_crt_composer *)CC_ALLOC_WS(ws, struct_cche_crt_composer_nof_n(1));
        CC_TEST_DISABLE_DIT_ASSERTS
        is(cche_crt_composer_init_ws(ws, composer, 1, plaintext_modulus), CCERR_OK, "ccche_crt_composer_init != CCERR_OK");
        CC_TEST_ENABLE_DIT_ASSERTS
        ...
        is(cche_crt_composer_compose_ws(ws, composer, len, values_crt_decoded1, value1),
           ...
           "cche_crt_composer_compose != CCERR_OK");
        CC_TEST_DISABLE_DIT_ASSERTS
        is(cche_crt_compose_ws(ws, len, values_crt_decoded2, value1, 1, plaintext_modulus),
           CCERR_OK,
           "cche_crt_compose != CCERR_OK");
        CC_TEST_ENABLE_DIT_ASSERTS
        is(array_eq_int64(CC_ARRAY_LEN(values), values, values_crt_decoded1), true, "test_cche_crt_decode");
...
int cche_tests(TM_UNUSED int argc, TM_UNUSED char *const *argv)
{
    ...
    ntests += 9 * CCHE_SCHEMES_COUNT;    // test_cche_encode_decode_simd_int64_out_of_bounds
    ...
    ntests += 62 * CCHE_SCHEMES_COUNT;   // test_cche_compose_decompose
    ntests += 55;                        // test_cche_serialization
    ntests += 22 * CCHE_SCHEMES_COUNT;
    ...
    for (uint32_t he_scheme = 1; he_scheme <= CCHE_SCHEMES_COUNT; ++he_scheme) {
        ...
        test_cche_encode_simd_reduced_int64(he_scheme);
        test_cche_crt_decode(he_scheme);
