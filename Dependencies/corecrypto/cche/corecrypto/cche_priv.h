...
/// @brief Compose CRT submods together
/// @param nresults length of the input vectors (and output vector)
/// @param results the variable to catch results write-back
/// @param values the CRT submods, `values` is a vector of `nparams` elements, each of which is a `nresults`-element vector of
/// mods in central representation in [-floor(p/2), floor(p-1)/2], p is the corresponding modulus.
/// @param nplaintext_moduli number of plaintext moduli in CRT
/// @param plaintext_moduli a vector of `nparams` plaimtext moduli.
/// @return`CCERR_OK` if the operation was successful
/// // @details this function is not constant time and may leak plaintext moduli via timing.
