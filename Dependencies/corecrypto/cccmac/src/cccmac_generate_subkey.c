/* Copyright (c) (2013-2019,2022,2024) Apple Inc. All rights reserved.
 *
...
int cccmac_generate_subkeys(const struct ccmode_cbc *cbc, size_t key_nbytes, const void *key, uint8_t *subkey1, uint8_t *subkey2)
{
    ...
    int rv;
    if ((rv = cccbc_one_shot_explicit_internal(cbc, key_nbytes, CMAC_BLOCKSIZE, cbc->block_size, 1, key, iv, L, L))) {
        return rv;
    }
