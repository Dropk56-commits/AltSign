/* Copyright (c) (2013-2019,2021,2022,2024) Apple Inc. All rights reserved.
 *
                         ...
                         size_t key_nbytes, const void *key_data)
{
    ...
    cccbc_init_internal(cbc, cccmac_mode_sym_ctx(cbc, ctx), key_nbytes, key_data);
    const uint8_t zeros[CMAC_BLOCKSIZE] = { 0 };
    cccbc_set_iv_internal(cbc, cccmac_mode_iv(cbc, ctx), zeros);
    cccmac_cbc(ctx)=cbc;
                 ...
                 size_t key_nbytes, const void *key_data)
{
    ...
    return cccmac_init_internal(cbc, ctx, key_nbytes, key_data);
}
