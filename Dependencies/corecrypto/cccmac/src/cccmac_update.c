/* Copyright (c) (2013,2016-2021,2024) Apple Inc. All rights reserved.
 *
...
int cccmac_update_internal(cccmac_ctx_t ctx, size_t data_nbytes, const void *data)
{
    ...
    if (cccmac_block_nbytes(ctx) > 0) {
        ...
        cccbc_update_internal(cbc, cccmac_mode_sym_ctx(cbc, ctx),
    ...
    while (nblocks) {
        ...
        cccbc_update_internal(cbc, cccmac_mode_sym_ctx(cbc, ctx), cccmac_mode_iv(cbc, ctx), process_nblocks, in, tmp);
...
int cccmac_update(cccmac_ctx_t ctx, size_t data_nbytes, const void *data)
{
    ...
    return cccmac_update_internal(ctx, data_nbytes, data);
}
