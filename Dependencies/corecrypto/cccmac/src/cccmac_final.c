/* Copyright (c) (2010,2011,2013-2019,2021,2024) Apple Inc. All rights reserved.
 *
...
int cccmac_final_generate_internal(cccmac_ctx_t ctx,
    ...
    cccbc_update_internal(cbc,cccmac_mode_sym_ctx(cbc,ctx),cccmac_mode_iv(cbc, ctx),
...
int cccmac_final_generate(cccmac_ctx_t ctx,
    ...
    return cccmac_final_generate_internal(ctx, mac_nbytes, mac);
}
...
int cccmac_final_verify_internal(cccmac_ctx_t ctx,
    ...
    int status;
    uint8_t full_mac[CMAC_BLOCKSIZE];
    status=cccmac_final_generate_internal(ctx,CMAC_BLOCKSIZE,full_mac);
    if (status != 0) {
        return status; // Computation error
    }
    if (cc_cmp_safe_internal(expected_mac_nbytes, expected_mac, full_mac) == 0) {
...
int cccmac_final_verify(cccmac_ctx_t ctx,
    ...
    return cccmac_final_verify_internal(ctx, expected_mac_nbytes, expected_mac);
}
