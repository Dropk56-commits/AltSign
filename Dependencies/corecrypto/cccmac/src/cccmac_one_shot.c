/* Copyright (c) (2013,2015-2019,2021,2024) Apple Inc. All rights reserved.
 *
...
int cccmac_one_shot_generate_internal(const struct ccmode_cbc *cbc,
    ...
    status=cccmac_init_internal(cbc, cmac, key_nbytes, key);
    cc_require(status==0,errOut);
    status=cccmac_update_internal(cmac, data_nbytes, data);
    cc_require(status==0,errOut);
    status=cccmac_final_generate_internal(cmac, mac_nbytes, mac);
    cc_require(status==0,errOut);
errOut:
...
int cccmac_one_shot_generate(const struct ccmode_cbc *cbc,
    ...
    return cccmac_one_shot_generate_internal(cbc, key_nbytes, key, data_nbytes, data, mac_nbytes, mac);
}
...
int cccmac_one_shot_verify(const struct ccmode_cbc *cbc,
    ...
    status=cccmac_update_internal(cmac, data_nbytes, data);
    cc_require(status==0,errOut);
    status=cccmac_final_verify_internal(cmac, mac_nbytes, mac);
    cc_require(status==0,errOut);
errOut:
    cccmac_mode_clear(cbc, cmac);
    return status;
