...
static int ccrns_modulus_init_helper_ws(cc_ws_t ws, ccrns_modulus_t p, ccrns_int value, ccpolyzp_po2cyc_runtime_t runtime)
{
    ...
    cc_unit has_remainder = cc_nonzero_mask_ccrns_int(ccpolyzp_po2cyc_units_to_rns_int(remainder));
...
int64_t ccpolyzp_po2cyc_rem_to_centered(ccrns_int x, ccrns_int p)
{
    ...
    ccrns_int m = cc_bit_to_mask_ccrns_int((x - (p_div_2 + 1)) >> CCRNS_INT_NBITS_MINUS_1);

    return (int64_t)(x - (p & ~m));
}
...
ccrns_int ccpolyzp_po2cyc_centered_to_rem(int64_t x, ccrns_int p)
{
    ...
    return (ccrns_int)x + (p & (ccrns_int)cc_neg_mask_int64(x));
}

ccrns_int ccpolyzp_po2cyc_int_to_rem(int64_t x, ccrns_modulus_const_t p)
{
    ccrns_int m = (ccrns_int)cc_neg_mask_int64(x);

    // store abs(x) in x_rns in constant time
    ccrns_int x_rns;
    cc_mux_mask(x_rns, m, (ccrns_int)(-x), (ccrns_int)x);

    x_rns = ccpolyzp_po2cyc_scalar_mod1(x_rns, p);
    ccrns_int neg_x = ccpolyzp_po2cyc_scalar_negate_mod(x_rns, p->value);

    ccrns_int result;
    cc_mux_mask(result, m, neg_x, x_rns);
    return result;
}

int64_t ccpolyzp_po2cyc_int_to_centered(int64_t x, ccrns_modulus_const_t p)
{
    ccrns_int x_rem = ccpolyzp_po2cyc_int_to_rem(x, p);
    return ccpolyzp_po2cyc_rem_to_centered(x_rem, p->value);
}
