...
#define cc_bit_to_mask_ccrns_int cc_bit_to_mask_uint64
#define cc_nonzero_mask_ccrns_int cc_nonzero_mask_uint64

CC_INLINE CC_WARN_RESULT
ccrns_int cc_neg_mask_ccrns_int(ccrns_int x)
{
    return (ccrns_int)cc_neg_mask_int64((int64_t)x);
}
...
CC_INLINE CC_WARN_RESULT ccrns_int ccpolyzp_po2cyc_scalar_cond_sub(ccrns_int x, ccrns_int p)
{
    ...
    ccrns_int mask = cc_neg_mask_ccrns_int(x_minus_p);
    return x_minus_p + (p & mask);
}
...
{
    cc_assert(x < p);
    return (p - x) & cc_nonzero_mask_ccrns_int(x);
}
...
/// @brief Transforms x from int64_t to remainder representation
/// @param p Modulus. Must be < 2^63
/// @return x % p in [0, p - 1]
/// @details Constant-time.
/// Note, `p < 2^63` comes from internal use of ccpolyzp_po2cyc_scalar_mod1
