...
static void test_ccpolyzp_po2cyc_int_to_rem(void)
{
    // odd modulus
    {
        CC_DECL_WORKSPACE_TEST(ws);
        struct ccrns_modulus p;
        is(ccrns_modulus_init_ws(ws, &p, 7), CCERR_OK, "ccrns_modulus_init != CCERR_OK");
        is(ccpolyzp_po2cyc_int_to_rem(0, &p), 0, "int_to_rem(0, 7) != 0");
        is(ccpolyzp_po2cyc_int_to_rem(1 + 7, &p), 1, "int_to_rem(8, 7) != 1");
        is(ccpolyzp_po2cyc_int_to_rem(2 + 14, &p), 2, "int_to_rem(16, 7) != 2");
        is(ccpolyzp_po2cyc_int_to_rem(3 + 21, &p), 3, "int_to_rem(24, 7) != 3");
        is(ccpolyzp_po2cyc_int_to_rem(-3 - 21, &p), 4, "int_to_rem(-24, 7) != 4");
        is(ccpolyzp_po2cyc_int_to_rem(-2 - 14, &p), 5, "int_to_rem(-16, 7) != 5");
        is(ccpolyzp_po2cyc_int_to_rem(-1 - 7, &p), 6, "int_to_rem(-8, 7) != 6");
        CC_FREE_WORKSPACE(ws);
    }
    // even modulus
    {
        CC_DECL_WORKSPACE_TEST(ws);
        struct ccrns_modulus p;
        is(ccrns_modulus_init_ws(ws, &p, 8), CCERR_OK, "ccrns_modulus_init != CCERR_OK");
        is(ccpolyzp_po2cyc_int_to_rem(0, &p), 0, "int_to_rem(0, 8) != 0");
        is(ccpolyzp_po2cyc_int_to_rem(1 + 8, &p), 1, "int_to_rem(9, 8) != 1");
        is(ccpolyzp_po2cyc_int_to_rem(2 + 16, &p), 2, "int_to_rem(18, 8) != 2");
        is(ccpolyzp_po2cyc_int_to_rem(3 + 24, &p), 3, "int_to_rem(27, 8) != 3");
        is(ccpolyzp_po2cyc_int_to_rem(-4 - 32, &p), 4, "int_to_rem(-36, 8) != 4");
        is(ccpolyzp_po2cyc_int_to_rem(-3 - 24, &p), 5, "int_to_rem(-27, 8) != 5");
        is(ccpolyzp_po2cyc_int_to_rem(-2 - 16, &p), 6, "int_to_rem(-18, 8) != 6");
        is(ccpolyzp_po2cyc_int_to_rem(-1 - 8, &p), 7, "int_to_rem(-9, 8) != 7");
        CC_FREE_WORKSPACE(ws);
    }
}

static void test_ccpolyzp_po2cyc_int_to_centered(void)
{
    // odd modulus
    {
        CC_DECL_WORKSPACE_TEST(ws);
        struct ccrns_modulus p;
        is(ccrns_modulus_init_ws(ws, &p, 7), CCERR_OK, "ccrns_modulus_init != CCERR_OK");
        is(ccpolyzp_po2cyc_int_to_centered(0, &p), 0, "int_to_centered(0, 7) != 0");
        is(ccpolyzp_po2cyc_int_to_centered(1 + 7, &p), 1, "int_to_centered(8, 7) != 1");
        is(ccpolyzp_po2cyc_int_to_centered(2 + 14, &p), 2, "int_to_centered(16, 7) != 2");
        is(ccpolyzp_po2cyc_int_to_centered(3 + 21, &p), 3, "int_to_centered(24, 7) != 3");
        is(ccpolyzp_po2cyc_int_to_centered(-3 - 21, &p), -3, "int_to_centered(-24, 7) != -3");
        is(ccpolyzp_po2cyc_int_to_centered(-2 - 14, &p), -2, "int_to_centered(-16, 7) != -2");
        is(ccpolyzp_po2cyc_int_to_centered(-1 - 7, &p), -1, "int_to_centered(-8, 7) != -1");
        CC_FREE_WORKSPACE(ws);
    ...
    // even modulus
    {
        ...
        struct ccrns_modulus p;
        is(ccrns_modulus_init_ws(ws, &p, 8), CCERR_OK, "ccrns_modulus_init != CCERR_OK");
        is(ccpolyzp_po2cyc_int_to_centered(0, &p), 0, "int_to_centered(0, 8) != 0");
        is(ccpolyzp_po2cyc_int_to_centered(1 + 8, &p), 1, "int_to_centered(9, 8) != 1");
        is(ccpolyzp_po2cyc_int_to_centered(2 + 16, &p), 2, "int_to_centered(18, 8) != 2");
        is(ccpolyzp_po2cyc_int_to_centered(3 + 24, &p), 3, "int_to_centered(27, 8) != 3");
        is(ccpolyzp_po2cyc_int_to_centered(-4 - 32, &p), -4, "int_to_centered(-36, 8) != -4");
        is(ccpolyzp_po2cyc_int_to_centered(-3 - 24, &p), -3, "int_to_centered(-27, 8) != -3");
        is(ccpolyzp_po2cyc_int_to_centered(-2 - 16, &p), -2, "int_to_centered(-18, 8) != -2");
        is(ccpolyzp_po2cyc_int_to_centered(-1 - 8, &p), -1, "int_to_centered(-9, 8) != -1");
        CC_FREE_WORKSPACE(ws);
...
void test_ccpolyzp_po2cyc_scalar(void)
{
    ...
    test_ccpolyzp_po2cyc_int_to_rem();
    test_ccpolyzp_po2cyc_int_to_centered();
