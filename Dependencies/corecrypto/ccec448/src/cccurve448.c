                         ...
                         struct ccrng_state *rng)
{
    ...
    if (cc_cmp_safe_internal(56, out, zeros) == 0) {
        rv = CCERR_PARAMETER;
    }
...
static int cccurve448_make_priv_internal(struct ccrng_state *rng, ccec448secretkey sk)
{
    CC_ASSERT_DIT_IS_ENABLED
...
int cccurve448_make_priv(struct ccrng_state *rng, ccec448secretkey sk)
{
    ...
    return cccurve448_make_priv_internal(rng, sk);
}

static int cccurve448_make_pub_internal(struct ccrng_state *rng, ccec448pubkey pk, const ccec448secretkey sk)
{
    CC_ASSERT_DIT_IS_ENABLED
    
    ccec_const_cp_t cp = ccec_cp_x448();
    return cccurve448_internal(cp, pk, sk, kX448BasePoint, rng);
}
...
int cccurve448_make_pub(struct ccrng_state *rng, ccec448pubkey pk, const ccec448secretkey sk)
{
    ...
    return cccurve448_make_pub_internal(rng, pk, sk);
}
...
int cccurve448_make_key_pair(struct ccrng_state *rng, ccec448pubkey pk, ccec448secretkey sk)
{
    ...
    int rv = cccurve448_make_priv_internal(rng, sk);
    cc_require_or_return(rv == CCERR_OK, rv);
