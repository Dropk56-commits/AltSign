...
static CC_NORETURN void usage(char **argv)
{
    ...
    print_family_list();
    printf("\n\nUsage Suggestion:\n");
    printf("Frequently we want to know performance benchmarks on performant or energy preserving cores.\n");
    printf("Use clpcctrl -g (p_only|e_only) -f (<mhz>|min|max)\n on the device in question before running ");
    printf("corecrypto_perf, to define core type and frequency of cores:\n");
    printf("\t* p_only & e_only specifies performant vs. efficient cores, and\n");
    printf("\t* max, min or specified MHZ define the core frequency.\n");
    exit(0);
}
#endif
