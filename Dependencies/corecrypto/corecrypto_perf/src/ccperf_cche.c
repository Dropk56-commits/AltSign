...
static double perf_cche_crt_composer(size_t loops, uint32_t degree, cche_scheme_t he_scheme)
{
    if ((he_scheme != CCHE_SCHEME_BFV) && (he_scheme != CCHE_SCHEME_BGV)) {
        cc_abort("Invalid he scheme");
    }
    uint64_t plaintext_moduli[2] = { 40961, 66537 };
    int64_t *values_submod = calloc(CC_ARRAY_LEN(plaintext_moduli) * degree, sizeof(int64_t));
    for (uint32_t i = 0; i < degree; ++i) {
        values_submod[i] = i % (int64_t)((plaintext_moduli[0] - 1) >> 1);
        values_submod[i + degree] = i % (int64_t)((plaintext_moduli[1] - 1) >> 1);
    }
    int64_t *values_crt_composed = calloc(degree, sizeof(int64_t));
    perf_start();
    do {
        if (cche_crt_compose(degree, values_crt_composed, values_submod, CC_ARRAY_LEN(plaintext_moduli), plaintext_moduli) !=
            CCERR_OK) {
            abort();
        }
    } while (--loops != 0);
