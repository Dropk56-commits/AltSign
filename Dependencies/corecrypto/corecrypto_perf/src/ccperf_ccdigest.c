/* Copyright (c) (2011,2012,2014-2016,2018,2019,2021-2024) Apple Inc. All rights reserved.
 *
...
#if defined(__arm64__) && CC_DARWIN
static struct ccdigest_info ccsha512_vng_arm_di_if_supported;
static struct ccdigest_info ccsha512_vng_arm_hw_di_if_supported;
#endif
...
#if defined(__arm64__) && CC_DARWIN
    CCDIGEST_TEST(ccsha512_vng_arm_di_if_supported),
    CCDIGEST_TEST(ccsha512_vng_arm_hw_di_if_supported),
#endif
...
#if defined(__arm64__) && CC_DARWIN
static int cpu_has_sha512_acceleration(void) {
    static int feat_sha512 = -1;
    if (feat_sha512 == -1) {
        size_t sz_feat_sha512 = sizeof(feat_sha512);
        if (sysctlbyname("hw.optional.arm.FEAT_SHA512", &feat_sha512, &sz_feat_sha512, NULL, 0) == -1) {
            feat_sha512 = 0;
        }
    }
    return feat_sha512;
...
static double perf_ccdigest(size_t loops, size_t *psize, const void *arg)
{
    ...
    // This will happen on platforms that do not support the hardware-conditional tests.
    if (test->di->oid == NULL) {
        return 0;
    }
...
{
    // Hardware-conditional perf tests
#if defined(__arm64__) && CC_DARWIN
    cc_clear(sizeof(ccsha512_vng_arm_di_if_supported), &ccsha512_vng_arm_di_if_supported);
    cc_clear(sizeof(ccsha512_vng_arm_hw_di_if_supported), &ccsha512_vng_arm_hw_di_if_supported);
#if CC_USE_ASM && CCSHA2_VNG_ARM
    memcpy(&ccsha512_vng_arm_di_if_supported,&ccsha512_vng_arm_di,sizeof(ccsha512_vng_arm_di_if_supported));

    if (cpu_has_sha512_acceleration()) {
        memcpy(&ccsha512_vng_arm_hw_di_if_supported,&ccsha512_vng_arm_hw_di,sizeof(ccsha512_vng_arm_hw_di_if_supported));
    }
#endif
#endif
