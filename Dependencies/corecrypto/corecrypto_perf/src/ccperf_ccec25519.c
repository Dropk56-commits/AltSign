/* Copyright (c) (2014-2016,2018,2019,2022-2024) Apple Inc. All rights reserved.
 *
                                           ...
                                           CC_UNUSED size_t *size, CC_UNUSED const struct ccdigest_info *di)
{
    ...
    perf_start();
    do {
        rv = cccurve25519_make_pub(pk, sk);
        if (rv != CCERR_OK) {
            cc_abort("Failure in cccurve25519_make_pub");
        }
    } while (--loops != 0);
    return perf_seconds();
}
                                         ...
                                         CC_UNUSED size_t *size, CC_UNUSED const struct ccdigest_info *di)
{
    ...
    perf_start();
    do {
        rv = cccurve25519_make_key_pair(rng, pk, sk);
        if (rv != CCERR_OK) {
            cc_abort("Failure in cccurve25519_make_key_pair");
        }
    } while (--loops != 0);
    return perf_seconds();
}

static double perf_cccurve25519(size_t loops,
                                CC_UNUSED size_t *size, CC_UNUSED const struct ccdigest_info *di)
{
    ccec25519secretkey sk1;
    ccec25519pubkey pk1;
    ccec25519secretkey sk2;
    ccec25519pubkey pk2;
    ccec25519key out;
    
    int rv = 0;
    rv = cccurve25519_make_key_pair(rng, pk1, sk1);
    cc_assert(rv == CCERR_OK);
    ...
    perf_start();
    do {
        rv = cccurve25519_with_rng(rng, out, sk1, pk2);
        if (rv != CCERR_OK) {
            cc_abort("Failure in cccurve25519");
        }
    } while (--loops != 0);
    return perf_seconds();
}
        ...
        CC_UNUSED size_t *psize, const struct ccdigest_info *di)
{
    ...
    perf_start();
    do {
        rv = cced25519_make_key_pair(di, rng, pk, sk);
        if (rv != CCERR_OK) {
            cc_abort("Failure in cced25519_make_key_pair");
        }
    } while (--loops != 0);
    return perf_seconds();
}
        ...
        size_t *psize, const struct ccdigest_info *di)
{
    ...
    do {
        ...
        if (status) {
            cc_abort("Failure in cced25519_sign");
        }
    } while (--loops != 0);
    return perf_seconds();
}
        ...
        size_t *psize, const struct ccdigest_info *di)
{
    ...
    do {
        ...
        if (status) {
            cc_abort("Failure in cced25519_verify");
        }
    } while (--loops != 0);
    return perf_seconds();
}
...
static struct ccec25519_perf_test {
    ...
    _TEST(cccurve25519,ccsha512_ltc_di),
