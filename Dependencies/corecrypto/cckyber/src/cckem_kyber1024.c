...
static int cckem_kyber1024_derive_key_from_seed(cckem_full_ctx_t ctx,
                                                size_t seed_nbytes,
                                                const uint8_t *seed,
                                                struct ccrng_state *masking_rng)
{
    cc_require_or_return(seed_nbytes >= CCKYBER_SYM_NBYTES, CCERR_PARAMETER);

    uint8_t *pubkey = cckem_ctx_pubkey(cckem_public_ctx(ctx));
    uint8_t *privkey = cckem_ctx_privkey(ctx);

    // Generate a truly random `z` (the rejection key).
    uint8_t z[CCKYBER_SYM_NBYTES];
    int rv = ccrng_generate(masking_rng, CCKYBER_SYM_NBYTES, z);
    cc_require(rv == CCERR_OK, errOut);

    // Pass the seed as `d`.
    rv = cckyber_kem_keypair_coins(&cckyber1024_params, pubkey, privkey, seed, z);
    cc_require(rv == CCERR_OK, errOut);
...
static const struct cckem_info cckem_kyber1024_info = {
    ...
    .derive_key_from_seed = cckem_kyber1024_derive_key_from_seed,
