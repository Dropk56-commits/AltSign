...
int cckyber_kem_keypair_coins(const cckyber_params_t *params,
                              uint8_t *pubkey,
                              uint8_t *privkey,
                              const uint8_t d[CCKYBER_SYM_NBYTES],
                              const uint8_t z[CCKYBER_SYM_NBYTES])
{
    int rv = cckyber_indcpa_keypair(params, pubkey, privkey, d);
    ...
    cc_memcpy(privkey + CCKYBER_PRIVKEY_NBYTES(params) - CCKYBER_SYM_NBYTES, z, CCKYBER_SYM_NBYTES);
                        ...
                        struct ccrng_state *rng)
{
    ...
    const uint8_t *z = coins + CCKYBER_SYM_NBYTES;

    rv = cckyber_kem_keypair_coins(params, pubkey, privkey, d, z);
    cc_require(rv == CCERR_OK, errOut);
                                      ...
                                      uint8_t *sk)
{
    ...
    int fail = cc_cmp_safe_internal(CCKYBER_EK_NBYTES(params), ek, cmp);
