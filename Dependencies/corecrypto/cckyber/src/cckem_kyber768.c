...
static int cckem_kyber768_derive_key_from_seed(cckem_full_ctx_t ctx,
                                               size_t seed_nbytes,
                                               const uint8_t *seed,
                                               struct ccrng_state *masking_rng)
{
    ...
    // Pass the seed as `d`.
    rv = cckyber_kem_keypair_coins(&cckyber768_params, pubkey, privkey, seed, z);
    cc_require(rv == CCERR_OK, errOut);
...
static const struct cckem_info cckem_kyber768_info = {
    ...
    .derive_key_from_seed = cckem_kyber768_derive_key_from_seed,
