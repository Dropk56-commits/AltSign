/* Copyright (c) (2012,2014-2022,2024) Apple Inc. All rights reserved.
 *
...
static int tests_run_index(int i, int argc, char * const *argv, byteBuffer seed)
{
    ...
    if (!cc_test_dit_asserts_enabled())
    {
        fprintf(stderr, "\nDIT Asserts should be enabled! \n");
        testlist[i].failed_tests = testlist[i].planned_tests;
        return -1;
    }
#endif
    run_one_test(&testlist[i], argc, argv, seed->len, seed->bytes);
