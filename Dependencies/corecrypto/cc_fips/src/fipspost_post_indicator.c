...
int fipspost_post_indicator(CC_UNUSED uint32_t fips_mode)
{
    ...
    success &= !fips_allowed1(ccansikdf_x963, ccsha512_256_di);
    ...
    /// SS
#if !(CC_USE_L4 || CC_KERNEL)
    success &= !fips_allowed0(ccss_shamir_parameters_init);
    success &= !fips_allowed0(ccss_shamir_share_generator_init);
    success &= !fips_allowed0(ccss_shamir_share_generator_generate_share);
#endif
