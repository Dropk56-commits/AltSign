...
#define FIPS_RSA_PAIRWISE_KEY_N ccn_nof(RSA_NBITS)
...
CC_INLINE int fipspost_post_rsa_sign(uint32_t fips_mode, const ccrsa_full_ctx_t full_key)
{
    ...
    int rv = ccrsa_sign_pkcs1v15_msg_blinded(&ccrng_zero, full_key, ccsha256_di(), MESSAGE_LEN, MESSAGE, &siglen, sig);
...
CC_INLINE int fipspost_post_rsa_verify(uint32_t fips_mode, const ccrsa_full_ctx_t full_key)
{
...
#if !(CC_USE_L4 || CC_DISABLE_RSAKEYGEN)
CC_INLINE int fipspost_post_rsa_pairwise(const ccrsa_full_ctx_t full_key)
{
    ...
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCRSA_PAIRWISE_CONSISTENCY_CHECK_WORKSPACE_N(ccn_sizeof(RSA_NBITS)));
    // We first must verify that the consistency test passes.
    rv = ccrsa_pairwise_consistency_check_ws(ws, full_key, &ccrng_zero);
    if (rv) {
        failf("[PCT] CCRSA_PAIRWISE_CONSISTENCY: unexpected FAILURE");
    } else {
        debugf("[PCT] CCRSA_PAIRWISE_CONSISTENCY: expected SUCCESS");
    }

    // FORCEFAIL newly generated Key
    cc_unit *m = ccrsa_ctx_m(full_key);
    ccn_add1(ccrsa_ctx_n(full_key), m, m, 0x01);

    // We then must verify that the consistency test fails once the key has been modified.
    rv = ccrsa_pairwise_consistency_check_ws(ws, full_key, &ccrng_zero);
    if (rv) {
        debugf("[PCT] CCRSA_PAIRWISE_CONSISTENCY: FORCEFAIL");
        rv = CCERR_OK;
    } else {
        failf("[PCT] CCRSA_PAIRWISE_CONSISTENCY: unexpected SUCCESS");
        rv = CCERR_INTERNAL;
    }
...
#endif // !(CC_USE_L4 || CC_DISABLE_RSAKEYGEN)
...
int fipspost_post_rsa_sig(uint32_t fips_mode)
{
    ...
    rv |= fipspost_post_rsa_sign(fips_mode, full_key);
    rv |= fipspost_post_rsa_verify(fips_mode, full_key);
#if !(CC_USE_L4 || CC_DISABLE_RSAKEYGEN)
    if (FIPS_MODE_IS_FORCEFAIL(fips_mode)) {
        rv |= fipspost_post_rsa_pairwise(full_key);
    }
#endif // !(CC_USE_L4 || CC_DISABLE_RSAKEYGEN)
