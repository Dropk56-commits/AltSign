/* Copyright (c) (2010,2011,2014-2016,2018,2019,2021,2023,2024) Apple Inc. All rights reserved.
 *
...
void ccdigest_update_internal(const struct ccdigest_info *di, ccdigest_ctx_t ctx, size_t len, const void *data)
{
    ...
    while (len > 0) {
        if (ccdigest_num(di, ctx) == 0 && len > di->block_size) {
            ...
            size_t n = cc_min_size(di->block_size - ccdigest_num(di, ctx), len);
...
void ccdigest_update(const struct ccdigest_info *di, ccdigest_ctx_t ctx, size_t len, const void *data)
{
    ...
    return ccdigest_update_internal(di, ctx, len, data);
}
