/* Copyright (c) (2016-2022,2024) Apple Inc. All rights reserved.
 *
...
static int get_two_blocks_entropy(uint8_t *entropy)
{
    ...
    if (0 == cc_cmp_safe_internal(blk_len, entropy, entropy_ref) || (0 == cc_cmp_safe_internal(blk_len, entropy, &entropy[blk_len]))) {
...
static int predictionbreak_locked(struct ccrng_cryptographic_internal_state *rng)
{
    ...
    status = ccdrbg_reseed_internal(&g_ccrng_drbg_info, drbg_state, sizeof(entropy), entropy, sizeof(nonce), &nonce);
...
{
    // `Generate` is in an awkward spot where it's called directly by callers (via ccrng())
    // and internally during some subroutines (e.g. to generate masks). Those internal
    // callers will need to incur the costs of the speculation barrier unless we perform
    // serious plubming to create "_internal" variants of the RNG functions. For
    ...
    while ((nbytes > 0) && ((drbg_status == CCDRBG_STATUS_OK) || (drbg_status == CCDRBG_STATUS_NEED_RESEED))) {
        ...
        if (nbytes <= sizeof(rng->random.buf)) {
            ...
            if (nbytes > 0) {
                ...
                drbg_status = ccdrbg_generate_internal(&g_ccrng_drbg_info, drbg_state, sizeof(rng->random.buf), rng->random.buf, 0, NULL);
            ...
            drbg_status = ccdrbg_generate_internal(&g_ccrng_drbg_info, drbg_state, req_size, out, 0, NULL);
...
int ccrng_cryptographic_init_once(void)
{
    ...
    err = ccdrbg_df_bc_init_internal(df_ctx,
    ...
    uint64_t nonce = cc_get_nonce();
    err = ccdrbg_init_internal(drbg_info, (struct ccdrbg_state *)rng->drbg_state_buf, sizeof(entropy), entropy, sizeof(nonce), &nonce, 0, NULL);
