/* Copyright (c) (2018-2022,2024) Apple Inc. All rights reserved.
 *
...
int ccrng_fortuna_generate(struct ccrng_fortuna_ctx *ctx, size_t nbytes, void *out)
{
    ...
    ccctr_init_internal(aes256ctr_info, aes256ctr_ctx, sizeof(ctx->key), ctx->key, ctx->ctr);
    ccctr_update_internal(aes256ctr_info, aes256ctr_ctx, sizeof(ctx->key), zeros, ctx->key);

    ccctr_update_internal(aes256ctr_info, aes256ctr_ctx, nbytes, zeros, out);
...
static void addentropy(struct ccrng_fortuna_ctx *ctx, int pool_idx, size_t entropy_nbytes, const void *entropy, uint32_t nsamples)
{
    ...
    ccdigest_init_internal(sha256_info, sha256_ctx);

    const uint8_t label[] = CCRNG_FORTUNA_LABEL(CCRNG_FORTUNA_OP_ADDENTROPY);
    ccdigest_update_internal(sha256_info, sha256_ctx, sizeof(label), label);

    uint8_t id[sizeof(uint32_t)];
    cc_store32_be((uint32_t) pool_idx, id);
    ccdigest_update_internal(sha256_info, sha256_ctx, sizeof(id), id);

    ccdigest_update_internal(sha256_info, sha256_ctx, sizeof(pool->data), pool->data);

    uint64_t rand;
    (void)cc_rdrand(&rand);
    rand = CC_H2BE64(rand);
    ccdigest_update_internal(sha256_info, sha256_ctx, sizeof(rand), &rand);

    ccdigest_update_internal(sha256_info, sha256_ctx, entropy_nbytes, entropy);
...
static bool schedreseed(struct ccrng_fortuna_ctx *ctx, int pool_idx)
{
    ...
    ccdigest_update_internal(sha256_info, sha256_ctx, sizeof(sched), &sched);

    ccdigest_update_internal(sha256_info, sha256_ctx, sizeof(ctx->key), ctx->key);
