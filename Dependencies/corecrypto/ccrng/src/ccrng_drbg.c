/* Copyright (c) (2014-2016,2018,2019,2021,2024) Apple Inc. All rights reserved.
 *
                    ...
                    const void *seed)
{
    ...
    int err = ccdrbg_init_internal(drbg_info, drbg_state, length, seed, length, seed, sizeof(const_seed), const_seed);
    if (err != CCERR_OK) {
        return err;
    }
...
int ccrng_drbg_reseed(struct ccrng_drbg_state *rng, size_t entropylen, const void *entropy, size_t inlen, const void *in)
{
    ...
    return ccdrbg_reseed_internal(rng->drbg_info, rng->drbg_state, entropylen, entropy, inlen, in);
}
