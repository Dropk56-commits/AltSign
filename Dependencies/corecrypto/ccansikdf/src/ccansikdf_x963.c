                                 ...
                                 uint8_t *out)
{
    ...
    ccdigest_update_internal(di, working_ctx, sizeof(counter), counter);

    for (size_t j = 0; j < sharedinfo_count; j++) {
        ccdigest_update_internal(di, working_ctx, sharedinfo_inputs[j].nbytes, sharedinfo_inputs[j].base);
    }
                         ...
                         uint8_t *cc_counted_by(key_len) key)
{
    ...
    ccdigest_init_internal(di, Z_ctx);
    ccdigest_update_internal(di, Z_ctx, Z_nbytes, Z);
