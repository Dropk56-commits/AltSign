/* Copyright (c) (2011,2012,2014-2017,2019,2021-2024) Apple Inc. All rights reserved.
 *
...
int ccec_pairwise_consistency_check_ws(cc_ws_t ws, ccec_full_ctx_t full_key, struct ccrng_state *rng)
{
    ...
    CC_DECL_BP_WS(ws, bp);
    void *r = CC_ALLOC_WS(ws, n);
    void *s = CC_ALLOC_WS(ws, n);

    int rv = ccec_sign_composite_msg_ws(ws, full_key, ccsha256_di(), 0, &dummy, r, s, rng);
    cc_require(rv == CCERR_OK, errOut);

    cc_fault_canary_t canary;
    rv = ccec_verify_composite_msg_ws(ws, ccec_ctx_pub(full_key), ccsha256_di(), 0, &dummy, r, s, canary);
    cc_require(rv == CCERR_OK && CC_FAULT_CANARY_EQUAL_INTERNAL(CCEC_FAULT_CANARY, canary), errOut);
...
int ccec_compact_generate_key_checkverify_and_extract_ws(cc_ws_t ws, ccec_generate_key_ctx_t key, ccec_full_ctx_t *fkey)
{
    *fkey = NULL;
    ...
    cc_require_or_return(rv == CCERR_OK && CC_FAULT_CANARY_EQUAL_INTERNAL(CCEC_FAULT_CANARY, canary), rv);
