                       ...
                       uint8_t *s_out)
{
    ...
    if (r_out) {
        cc_require(ccn_write_uint_padded_ct_internal(n, r, ccec_signature_r_s_size(key), r_out) >= 0, out);
    }
    if (s_out) {
        cc_require(ccn_write_uint_padded_ct_internal(n, s, ccec_signature_r_s_size(key), s_out) >= 0, out);
    }
    result = CCERR_OK;
...
int ccec_verify(ccec_pub_ctx_t key, size_t digest_len, const uint8_t *digest, size_t sig_len, const uint8_t *sig, bool *valid)
{
    ...
    *valid = false;
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCEC_VERIFY_DIGEST_WORKSPACE_N(ccec_ctx_n(key)));
    int result = ccec_verify_digest_ws(ws, key, digest_len, digest, sig_len, sig, NULL);
    CC_FREE_WORKSPACE(ws);
