...
int ccec_export_affine_point_public_value(ccec_const_cp_t cp, int format, ccec_const_affine_point_t point, size_t *out_nbytes, uint8_t *out)
{
    ...
    *out_nbytes = expected_size;
    ...
    int rv = ccn_write_uint_padded_ct_internal(n, ccec_point_x(point, cp), ccec_cp_prime_size(cp), out_x);
    ...
    if ((format == CCEC_FORMAT_UNCOMPRESSED) || (format == CCEC_FORMAT_HYBRID)) {
        ...
        rv = ccn_write_uint_padded_ct_internal(n, ccec_point_y(point, cp), ccec_cp_prime_size(cp), out_y);
        cc_require_or_return(rv >= 0, CCERR_INTERNAL);
    }
...
int ccec_export_affine_point_internal(ccec_const_cp_t cp, int format, ccec_const_affine_point_t point, size_t *out_nbytes, uint8_t *out)
{
    CC_ASSERT_DIT_IS_ENABLED
    
    return ccec_export_affine_point_public_value(cp, format, point, out_nbytes, out);
}
...
int ccec_export_affine_point(ccec_const_cp_t cp, int format, ccec_const_affine_point_t point, size_t *out_nbytes, uint8_t *out)
{
    ...
    return ccec_export_affine_point_internal(cp, format, point, out_nbytes, out);
}
