/* Copyright (c) (2014,2015,2017-2019,2022,2024) Apple Inc. All rights reserved.
 *
                 ...
                 void *hash)
{
    ...
    ccdigest_init_internal(di, dictx);
    ccdigest_update_internal(di, dictx, 4, "\x00\x00\x00\x01");
    ccdigest_update_internal(di, dictx, skey_size, skey);

    /* params */
    ccdigest_update_internal(di, dictx, 1, &curve->curve_oid[0]);
    ccdigest_update_internal(di, dictx, curve->curve_oid[0], &curve->curve_oid[1]);
    ccdigest_update_internal(di, dictx, 1, &curve->public_key_alg);
    ccdigest_update_internal(di, dictx, 2, "\x03\x01");
    ccdigest_update_internal(di, dictx, 1, &wrap->kdfhash_id);
    ccdigest_update_internal(di, dictx, 1, &wrap->kek_id);
    ccdigest_update_internal(di, dictx, 20, "Anonymous Sender    ");
    ccdigest_update_internal(di, dictx, fingerprint_size, fingerprint);
