/* Copyright (c) (2014,2015,2018,2019,2021,2024) Apple Inc. All rights reserved.
 *
...
static bool ccec_is_compactable_point_ws(cc_ws_t ws, ccec_const_cp_t cp, ccec_const_affine_point_t point)
{
    ...
    if (ccn_cmp_public_value(n, t, ccec_point_y(point, cp)) < 0) {
...
int ccec_compact_export_pub(void *out, ccec_pub_ctx_t key)
{
    ...
    return ccec_export_affine_point_public_value(cp, CCEC_FORMAT_COMPACT, (ccec_const_affine_point_t)ccec_ctx_point(key), &out_nbytes, out);
}
