/* Copyright (c) (2014,2015,2018-2021,2024) Apple Inc. All rights reserved.
 *
...
int ccec_compact_export(const int fullkey, void *out, ccec_full_ctx_t key)
{
    ...
    if (fullkey) {
        ...
        rv = ccn_write_uint_padded_ct_internal(ccec_cp_n(cp), ccec_ctx_k(key), ccec_cp_order_size(cp), (uint8_t *)out + ccec_cp_prime_size(cp));
