                                ...
                                ccec_affine_point_t point)
{
    ...
    rv = ccn_read_uint_public_value(n, ccec_point_x(point, cp), ccec_cp_prime_size(cp), in_x);
    cc_require(rv == CCERR_OK, errOut);
    cc_require_action(ccn_cmp_public_value(n, ccec_point_x(point, cp), ccec_cp_p(cp)) == -1, errOut, rv = rv_encoding_error); // check x < p
    ...
    if (format == CCEC_FORMAT_COMPACT || format == CCEC_FORMAT_COMPRESSED) {
        ...
        rv = ccn_read_uint_public_value(n, ccec_point_y(point, cp), ccec_cp_prime_size(cp), in_y);
        cc_require(rv == CCERR_OK, errOut);
    }
    ...
    if (format == CCEC_FORMAT_COMPACT || format == CCEC_FORMAT_COMPRESSED) {
        ...
        cc_unit s;
        if (format == CCEC_FORMAT_COMPACT) {
            s = ccn_cmp_public_value(n, t, ccec_point_y(point, cp)) < 0; // convention for y is y = min(y', p-y');
