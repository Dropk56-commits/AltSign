...
int ccec_raw_import_pub(ccec_const_cp_t cp, size_t in_len, const uint8_t *in, ccec_pub_ctx_t key)
{
    ...
    cc_require(ccn_read_uint_public_value(n, ccec_ctx_x(key), step, in) == 0, err);
    cc_require(ccn_read_uint_public_value(n, ccec_ctx_y(key), step, in + step) == 0, err);
...
int ccec_raw_import_priv_only(ccec_const_cp_t cp, size_t in_len, const uint8_t *in, ccec_full_ctx_t key)
{
    ...
    return ccn_read_uint_internal(n, ccec_ctx_k(key), in_len, in);
}
