/* Copyright (c) (2014-2022,2024) Apple Inc. All rights reserved.
 *
...
int ccec_sign_composite_ws(cc_ws_t ws,
                                  ccec_full_ctx_t key,
                                  size_t digest_len,
                                  ...
                                  struct ccrng_state *rng)
{
    ...
    ccn_write_uint_padded_ct_internal(ccec_ctx_n(key), r, ccec_signature_r_s_size(ccec_ctx_pub(key)), sig_r);
    ccn_write_uint_padded_ct_internal(ccec_ctx_n(key), s, ccec_signature_r_s_size(ccec_ctx_pub(key)), sig_s);
...
int ccec_sign_composite_msg_ws(cc_ws_t ws,
                               ccec_full_ctx_t key,
                               ...
                               struct ccrng_state *rng)
{
    ...
    return ccec_sign_composite_ws(ws, key, di->output_size, digest, sig_r, sig_s, rng);
}
                            ...
                            struct ccrng_state *rng)
{
    ...
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCEC_SIGN_COMPOSITE_MSG_WORKSPACE_N(ccec_ctx_n(key)));
    int rv = ccec_sign_composite_msg_ws(ws,
                                        key,
                                        di,
                                        msg_len, msg,
                                        sig_r, sig_s,
