/* Copyright (c) (2012-2019,2021,2024) Apple Inc. All rights reserved.
 *
...
int ccec_der_import_priv_keytype(size_t length, const uint8_t * data, ccoid_t *oid, size_t *n)
{
    ...
    cc_require(ccder_decode_eckey_internal(&version, &priv_size, &priv_key, &key_oid, &pub_size, &pub_key, data, data + length), out);
...
int ccec_der_import_priv(ccec_const_cp_t cp, size_t length, const uint8_t *data, ccec_full_ctx_t full_key)
{
    ...
    CC_DECL_WORKSPACE_OR_FAIL(ws,
                              CC_MAX_EVAL(CCEC_X963_IMPORT_PUB_WORKSPACE_N(ccec_cp_n(cp)),
                                          CCEC_MAKE_PUB_FROM_PRIV_WORKSPACE_N(ccec_cp_n(cp)))
                              );
    ...
    cc_require(0 == ccn_read_uint_internal(ccec_cp_n(cp), ccec_ctx_k(full_key), priv_size, priv_key), out);
    
    /* pub_size is partially checked by import pub: being odd */
    if (pub_key && (pub_size / 8 >= 2 * ccec_cp_prime_size(cp) + 1)) {
        cc_require(CCERR_OK == ccec_import_pub_ws(ws, cp, pub_size / 8, pub_key, ccec_ctx_pub(full_key)), out);
        result = 0;
    } else {
        /* Calculate pub from priv if absent */
        result = ccec_make_pub_from_priv_ws(ws, cp, NULL, ccec_ctx_k(full_key), NULL, ccec_ctx_pub(full_key));
    }
out:
