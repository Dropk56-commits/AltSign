/* Copyright (c) (2010-2022,2024) Apple Inc. All rights reserved.
 *
                                   ...
                                   const cc_unit *xaffine)
{
    ...
    if (ccn_cmp_internal(n, ccec_const_point_x(ccec_cp_g(cp), cp), xaffine) == 0) {
#if CC_SMALL_CODE
                                      ...
                                      cc_fault_canary_t fault_canary_out)
{
    ...
    cc_require(((result = ccn_read_uint_internal(n, e, d_nbytes, digest)) >= 0), errOut);
    ...
    if (ccn_cmp_internal(n, ccec_point_x(mg, cp), cczp_prime(zq)) >= 0) {
        ccn_sub_ws(ws, n, ccec_point_x(mg, cp), ccec_point_x(mg, cp), cczp_prime(zq));
    }

    // Verify x = r
    if (ccn_cmp_internal(n, ccec_point_x(mg, cp), r) == 0) {
        result = CCERR_VALID_SIGNATURE;
    } else {
        result = CCERR_INVALID_SIGNATURE;
    }

    ccn_write_uint_padded_ct_internal(n, r, rsize, r_input);
    ccn_write_uint_padded_ct_internal(n, ccec_point_x(mg, cp), rsize, r_computed);
