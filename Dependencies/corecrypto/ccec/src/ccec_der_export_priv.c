...
static int ccec_der_export_priv_ws(cc_ws_t ws, ccec_full_ctx_t key, ccoid_t key_oid, int include_public, size_t out_len, void *out)
{
    ...
    int rv = ccn_write_uint_padded_ct_internal(n, ccec_ctx_k(key), priv_size, priv_key);
    ...
    uint8_t *tmp = ccder_encode_eckey_internal(priv_size, priv_key, key_oid, pub_size, pub_key, out, der_end);
