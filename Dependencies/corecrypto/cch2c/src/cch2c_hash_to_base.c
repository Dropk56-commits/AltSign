                              ...
                              cc_unit *u)
{
    ...
    cc_require(cchkdf_extract_internal(di, dst_nbytes, dst, data_nbytes, data, buf) == CCERR_OK, out);
    cc_require(cchkdf_expand_internal(di, di->output_size, buf, sizeof(hkdf_info), hkdf_info,
                             info->l, buf) == CCERR_OK, out);

    cc_require(ccn_read_uint_internal(2 * n, t, info->l, buf) == CCERR_OK, out);
                              ...
                              cc_unit *u)
{
    ...
    cc_require(cchkdf_extract_internal(di, dst_nbytes, dst, data_nbytes + 1, buf, buf) == CCERR_OK, out);
