/* Copyright (c) (2010-2012,2014-2024) Apple Inc. All rights reserved.
 *
...
int cccbc_init_internal(const struct ccmode_cbc *mode, cccbc_ctx *ctx, size_t key_len, const void *cc_sized_by(key_len) key);

int cccbc_copy_iv_internal(cccbc_iv *cc_sized_by(len) iv_ctx, const void *cc_sized_by(len) iv, size_t len);
int cccbc_clear_iv_internal(cccbc_iv *cc_sized_by(len) iv_ctx, size_t len);

cc_ptrcheck_unavailable() // Use cccbc_copy_iv() or cccbc_clear_iv() directly.
int cccbc_set_iv_internal(const struct ccmode_cbc *mode, cccbc_iv *iv_ctx, const void *iv);

int cccbc_update_internal(const struct ccmode_cbc *mode, const cccbc_ctx *ctx, cccbc_iv *iv, size_t nblocks, const void *cc_indexable in, void *cc_indexable out);

int cccbc_one_shot_explicit_internal(const struct ccmode_cbc *mode,
                                     size_t key_len,
                                     size_t iv_len,
                                     size_t block_size,
                                     size_t nblocks,
...
int ccecb_init_internal(const struct ccmode_ecb *mode, ccecb_ctx *ctx, size_t key_len, const void *cc_sized_by(key_len) key);

int ccecb_update_internal(const struct ccmode_ecb *mode, const ccecb_ctx *ctx, size_t nblocks, const void *cc_indexable in, void *cc_indexable out);

int ccgcm_init_internal(const struct ccmode_gcm *mode, ccgcm_ctx *ctx, size_t key_nbytes, const void *cc_sized_by(key_nbytes) key);

int ccgcm_init_with_iv_internal(const struct ccmode_gcm *mode, ccgcm_ctx *ctx, size_t key_nbytes, const void *cc_sized_by(key_nbytes) key, const void *cc_unsafe_indexable iv);

int ccgcm_set_iv_internal(const struct ccmode_gcm *mode, ccgcm_ctx *ctx, size_t iv_nbytes, const void *cc_sized_by(iv_nbytes) iv);

int ccgcm_inc_iv_internal(const struct ccmode_gcm *mode, ccgcm_ctx *ctx, void *cc_unsafe_indexable iv);

int ccgcm_aad_internal(const struct ccmode_gcm *mode, ccgcm_ctx *ctx, size_t nbytes, const void *cc_sized_by(nbytes) additional_data);

int ccgcm_update_internal(const struct ccmode_gcm *mode, ccgcm_ctx *ctx, size_t nbytes, const void *cc_sized_by(nbytes) in, void *cc_sized_by(nbytes) out);

int ccgcm_finalize_internal(const struct ccmode_gcm *mode, ccgcm_ctx *ctx, size_t tag_nbytes, void *cc_sized_by(tag_nbytes) tag);

int ccgcm_reset_internal(const struct ccmode_gcm *mode, ccgcm_ctx *ctx);

int ccgcm_one_shot_internal(const struct ccmode_gcm *mode,
                            size_t key_nbytes,
...
int ccccm_one_shot_internal(const struct ccmode_ccm *mode,
                            size_t key_len,
                            const void *cc_sized_by(key_len) key,
                            size_t nonce_len,
