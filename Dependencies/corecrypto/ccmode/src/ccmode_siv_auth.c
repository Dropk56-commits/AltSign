...
int ccmode_siv_auth(ccsiv_ctx *ctx,
    ...
    cccmac_one_shot_generate_internal(_CCMODE_SIV_CBC_MODE(ctx),
...
int ccmode_siv_auth_finalize(ccsiv_ctx *ctx,
    ...
    if (nbytes>=block_size) {
        ...
        cccmac_init_internal(cbc, cmac, _CCMODE_SIV_KEYSIZE(ctx)/2, _CCMODE_SIV_K1(ctx));
        ...
        cccmac_update_internal(cmac, head_nblocks*block_size, in);

        // 3) MAC the tailing bytes
        cccmac_update_internal(cmac, tail_nbytes, block);
        cccmac_final_generate_internal(cmac, block_size,V);
        ...
        cccmac_one_shot_generate_internal(cbc, _CCMODE_SIV_KEYSIZE(ctx)/2,_CCMODE_SIV_K1(ctx),
