...
static int ccccm_finalize_and_verify_tag_internal(const struct ccmode_ccm *mode,
                                                  ccccm_ctx *ctx,
                                                  ccccm_nonce *nonce_ctx,
                                                  const uint8_t *cc_indexable mac)
{
    ...
    rc = cc_cmp_safe_internal(CCMODE_CCM_KEY_MAC_LEN(nonce_ctx), outTag, mac) == 0 ? CCERR_OK : CCMODE_INTEGRITY_FAILURE;
                                  ...
                                  const uint8_t *cc_indexable mac)
{
    ...
    return ccccm_finalize_and_verify_tag_internal(mode, ctx, nonce_ctx, mac);
}
                   ...
                   void *cc_sized_by(mac_size) mac)
{
    ...
    return ccccm_one_shot_internal(mode,
                                   key_len, key,
                                   nonce_len, nonce,
                                   nbytes, in, out,
                                   adata_len, adata,
                           ...
                           const uint8_t *cc_sized_by(mac_tag_nbytes) mac_tag)
{
    ...
    if (rc == 0) {
        rc = ccccm_finalize_and_verify_tag_internal(mode, ctx, nonce_ctx, mac_tag);
    }
    ccccm_ctx_clear(mode->size, ctx);
    ccccm_nonce_clear(mode->nonce_size, nonce_ctx);
