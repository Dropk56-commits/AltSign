/* Copyright (c) (2017-2019,2021-2024) Apple Inc. All rights reserved.
 *
...
int cccbc_init_internal(const struct ccmode_cbc *mode,
               cccbc_ctx *ctx,
               size_t key_len,
...
int cccbc_init(const struct ccmode_cbc *mode,
               cccbc_ctx *ctx,
               size_t key_len,
               const void *cc_sized_by(key_len) key)
{
    ...
    return cccbc_init_internal(mode, ctx, key_len, key);
}

int cccbc_copy_iv_internal(cccbc_iv *cc_sized_by(len) iv_ctx,
                  ...
                  size_t len)
{
    ...
    return cccbc_copy_iv_internal(iv_ctx, iv, len);
}
...
int cccbc_set_iv_internal(const struct ccmode_cbc *mode, cccbc_iv *iv_ctx, const void *iv)
{
    ...
    if (iv) {
        return cccbc_copy_iv_internal(iv_ctx, iv, mode->block_size);
    }

    return cccbc_clear_iv_internal(iv_ctx, mode->block_size);
}
...
int cccbc_set_iv(const struct ccmode_cbc *mode, cccbc_iv *iv_ctx, const void *iv)
{
    ...
    return cccbc_set_iv_internal(mode, iv_ctx, iv);
}

int cccbc_update_internal(const struct ccmode_cbc *mode,
                          const cccbc_ctx *ctx,
                          cccbc_iv *iv,
                          size_t nblocks,
...
int cccbc_update(const struct ccmode_cbc *mode,
                 const cccbc_ctx *ctx,
                 cccbc_iv *iv,
                 size_t nblocks,
                 const void *cc_indexable in,
                 void *cc_indexable out)
{
    ...
    return cccbc_update_internal(mode, ctx, iv, nblocks, in, out);
}
                   ...
                   void *out)
{
    ...
    return cccbc_one_shot_explicit_internal(mode,
                                            key_len,
                                            iv_len,
                                            mode->block_size,
                                            nblocks,
                            ...
                            void *cc_sized_by(block_size * nblocks) out)
{
    ...
    return cccbc_one_shot_explicit_internal(mode, key_len, iv_len, block_size, nblocks, key, iv, in, out);
}
