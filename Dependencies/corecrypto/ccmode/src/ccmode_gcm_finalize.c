/* Copyright (c) (2011,2012,2015,2016,2018,2019,2021,2024) Apple Inc. All rights reserved.
 *
...
int ccmode_gcm_finalize(ccgcm_ctx *key, size_t tag_nbytes, void *tag)
{
    ...
    if (_CCMODE_GCM_ECB_MODE(key)->encdec == CCMODE_GCM_DECRYPTOR) {
        rc = cc_cmp_safe_internal(tag_nbytes, out_tag, tag);
        rc = CCMODE_INTEGRITY_FAILURE & cc_nonzero_mask_int(rc);
    }
