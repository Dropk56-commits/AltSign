/* Copyright (c) (2015-2019,2021,2022,2024) Apple Inc. All rights reserved.
 *
...
int ccgcm_init_internal(const struct ccmode_gcm *mode,
                        ccgcm_ctx *ctx,
                        size_t key_nbytes,
...
int ccgcm_init(const struct ccmode_gcm *mode,
               ccgcm_ctx *ctx,
               size_t key_nbytes,
               const void *cc_sized_by(key_nbytes) key)
{
    ...
    return ccgcm_init_internal(mode, ctx, key_nbytes, key);
}

int ccgcm_init_with_iv_internal(const struct ccmode_gcm *mode, ccgcm_ctx *ctx,
                                ...
                                const void *iv)
{
    ...
    rc = ccgcm_init_internal(mode, ctx, key_nbytes, key);
    if (rc == 0) rc = ccgcm_set_iv_internal(mode, ctx, CCGCM_IV_NBYTES, iv);
                       ...
                       const void *iv)
{
    ...
    return ccgcm_init_with_iv_internal(mode, ctx, key_nbytes, key, iv);
}

int ccgcm_set_iv_internal(const struct ccmode_gcm *mode,
                          ccgcm_ctx *ctx,
                          size_t iv_nbytes,
...
int ccgcm_set_iv(const struct ccmode_gcm *mode,
                 ccgcm_ctx *ctx,
                 size_t iv_nbytes,
                 const void *cc_sized_by(iv_nbytes) iv)
{
    ...
    return ccgcm_set_iv_internal(mode, ctx, iv_nbytes, iv);
}

int ccgcm_inc_iv_internal(CC_UNUSED const struct ccmode_gcm *mode, ccgcm_ctx *ctx, void *iv)
{
    CC_ASSERT_DIT_IS_ENABLED
...
int ccgcm_inc_iv(CC_UNUSED const struct ccmode_gcm *mode, ccgcm_ctx *ctx, void *iv)
{
    ...
    return ccgcm_inc_iv_internal(mode, ctx, iv);
}

int ccgcm_aad_internal(const struct ccmode_gcm *mode,
                       ccgcm_ctx *ctx,
                       size_t nbytes,
...
int ccgcm_aad(const struct ccmode_gcm *mode,
              ccgcm_ctx *ctx,
              size_t nbytes,
              const void *cc_sized_by(nbytes) additional_data)
{
    ...
    return ccgcm_aad_internal(mode, ctx, nbytes, additional_data);
}

int ccgcm_gmac(const struct ccmode_gcm *mode,
               ccgcm_ctx *ctx,
               size_t nbytes,
...
int ccgcm_update_internal(const struct ccmode_gcm *mode,
                          ccgcm_ctx *ctx,
                          size_t nbytes,
                          const void *cc_sized_by(nbytes) in,
...
int ccgcm_update(const struct ccmode_gcm *mode,
                 ccgcm_ctx *ctx,
                 size_t nbytes,
                 const void *cc_sized_by(nbytes) in,
                 void *cc_sized_by(nbytes) out)
{
    ...
    return ccgcm_update_internal(mode, ctx, nbytes, in, out);
}

int ccgcm_finalize_internal(const struct ccmode_gcm *mode,
                            ccgcm_ctx *ctx,
                            size_t tag_nbytes,
...
int ccgcm_finalize(const struct ccmode_gcm *mode,
                   ccgcm_ctx *ctx,
                   size_t tag_nbytes,
                   void *cc_sized_by(tag_nbytes) tag)
{
    ...
    return ccgcm_finalize_internal(mode, ctx, tag_nbytes, tag);
}
...
int ccgcm_reset(const struct ccmode_gcm *mode, ccgcm_ctx *ctx)
{
    ...
    return ccgcm_reset_internal(mode, ctx);
}
                            ...
                            size_t tag_nbytes, void *tag)
{
    ...
    rc=ccgcm_init_internal(mode, ctx, key_nbytes   ,key); cc_require(rc==0, errOut);
    rc=ccgcm_set_iv_internal(mode, ctx, iv_nbytes ,iv); cc_require(rc==0, errOut);
    rc=ccgcm_aad_internal(mode, ctx, adata_nbytes ,adata); cc_require(rc==0, errOut);
    rc=ccgcm_update_internal(mode, ctx, nbytes    , in, out); cc_require(rc==0, errOut);
    rc=ccgcm_finalize_internal(mode, ctx, tag_nbytes   ,tag); cc_require(rc==0, errOut);
                   ...
                   size_t tag_nbytes, void *tag)
{
    ...
    return ccgcm_one_shot_internal(mode, 
                                   key_nbytes, key,
                                   iv_nbytes, iv,
                                   adata_nbytes, adata,
                                   nbytes, in, out,
