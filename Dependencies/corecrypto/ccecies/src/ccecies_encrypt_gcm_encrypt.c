/* Copyright (c) (2018,2019,2021,2024) Apple Inc. All rights reserved.
 *
                                ...
                                uint8_t *mac_tag)
{
    ...
    status = ccgcm_init_internal(gcm_encrypt, gcm_ctx, ecies->key_length, gcm_key_iv);
    status |= ccgcm_set_iv_internal(gcm_encrypt, gcm_ctx, CCECIES_CIPHERIV_NBYTES, ecies_iv_data);
    if ((sharedinfo2 != NULL) && (sharedinfo2_nbytes > 0)) {
        status |= ccgcm_aad_internal(gcm_encrypt, gcm_ctx, sharedinfo2_nbytes, sharedinfo2);
    }
    status |= ccgcm_update_internal(gcm_encrypt, gcm_ctx, plaintext_nbytes, plaintext, ciphertext);
    ...
    status = ccgcm_finalize_internal(gcm_encrypt, gcm_ctx, ecies->mac_length, mac_tag);
#if CC_DEBUG_ECIES
