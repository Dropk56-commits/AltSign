                                ...
                                uint8_t *decrypted_message)
{
    ...
    status = ccgcm_init_internal(gcm_decrypt, gcm_ctx, ecies->key_length, gcm_key_iv);
    status |= ccgcm_set_iv_internal(gcm_decrypt, gcm_ctx, CCECIES_CIPHERIV_NBYTES, ecies_iv_data);
    if ((sharedinfo2 != NULL) && (sharedinfo2_nbytes > 0)) {
        status |= ccgcm_aad_internal(gcm_decrypt, gcm_ctx, sharedinfo2_nbytes, sharedinfo2);
    }
    status |= ccgcm_update_internal(gcm_decrypt, gcm_ctx, ciphertext_nbytes, ciphertext, decrypted_message);
    ...
    status = ccgcm_finalize_internal(gcm_decrypt, gcm_ctx, ecies->mac_length, computed_tag);
