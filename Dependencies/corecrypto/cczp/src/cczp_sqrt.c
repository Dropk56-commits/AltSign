                                        ...
                                        const cc_unit *c3, const cc_unit *c5)
{
    ...
    for (size_t k = c1; k >= 2; k--) {
        ...
        // r = CMOV(r * c, r, b == 1)
        cc_unit s = (cc_unit)ccn_cmp_internal(n, b, one) & 1;
...
static int cczp_sqrt_tonelli_shanks_ws(cc_ws_t ws, cczp_const_t zp, cc_unit *r, const cc_unit *x)
{
    ...
    while (cczp_is_quadratic_residue_ws(ws, zp, c4) == CCERR_OK) {
...
{
    // This is an internal function that expects DIT to be enabled
