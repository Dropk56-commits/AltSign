/* Copyright (c) (2010-2019,2021,2022,2024) Apple Inc. All rights reserved.
 *
...
static void init_ctr(struct ccdrbg_nistctr_state *drbg_ctx, ccctr_ctx *ctr_ctx)
{
    ...
    ccctr_init_internal(drbg_ctx->custom.ctr_info, ctr_ctx, drbg_ctx->custom.keylen, drbg_ctx->Key, drbg_ctx->V);
}

static void update_with_ctr(struct ccdrbg_nistctr_state *drbg_ctx, ccctr_ctx *ctr_ctx, const uint8_t *provided_data)
{
    ccctr_update_internal(drbg_ctx->custom.ctr_info, ctr_ctx, DRBG_CTR_KEYLEN(drbg_ctx), provided_data, drbg_ctx->Key);
    provided_data += DRBG_CTR_KEYLEN(drbg_ctx);

    ccctr_update_internal(drbg_ctx->custom.ctr_info, ctr_ctx, DRBG_CTR_BLOCKLEN(drbg_ctx), provided_data, drbg_ctx->V);
}
...
static int generate(struct ccdrbg_state *ctx, size_t out_nbytes, void *out, size_t additional_nbytes, const void *additional)
{
    ...
    while (out_nbytes > 0) {
        ...
        ccctr_update_internal(drbg_ctx->custom.ctr_info, ctr_ctx, nbytes, zeros, out_bytes);
        out_nbytes -= nbytes;
        out_bytes += nbytes;
    }
    ...
    ccctr_update_internal(drbg_ctx->custom.ctr_info, ctr_ctx, remainder_nbytes, zeros, remainder);
