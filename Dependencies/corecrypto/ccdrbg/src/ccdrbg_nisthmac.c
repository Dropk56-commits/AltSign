/* Copyright (c) (2014-2019,2021-2024) Apple Inc. All rights reserved.
 *
...
static void update(struct ccdrbg_state *ctx, unsigned ndata, ...)
{
    ...
    for (uint8_t b = 0; b < 2; b += 1) {
        cchmac_init_internal(info, hmac_ctx, outlen, drbg_ctx->key);

        cchmac_update_internal(info, hmac_ctx, outlen, drbg_ctx->V);

        cchmac_update_internal(info, hmac_ctx, sizeof(b), &b);
        ...
        for (unsigned i = 0; i < ndata; i += 1) {
            ...
            cchmac_update_internal(info, hmac_ctx, nbytes, buf);
        ...
        cchmac_final_internal(info, hmac_ctx, drbg_ctx->key);

        cchmac_internal(info, outlen, drbg_ctx->key, outlen, drbg_ctx->V, drbg_ctx->V);
...
static int generate(struct ccdrbg_state *ctx, size_t out_nbytes, void *out, size_t add_nbytes, const void *add)
{
    ...
    while (out_nbytes > 0) {
        ...
        if (cc_cmp_safe_internal(outlen, Vprev, drbg_ctx->V) == 0) {
            done(ctx);
