       ...
       const void *data)
{
    ...
    if (*block_nbytes_left > 0 && data_nbytes >= block_nbytes_want) {
        ...
        err = cccbc_update_internal(cbc_info, cbc_ctx, cbc_iv, 1, block, block);
    ...
    while (data_nbytes >= CCAES_BLOCK_SIZE) {
        err = cccbc_update_internal(cbc_info, cbc_ctx, cbc_iv, 1, dp, block);
        cc_require(err == CCERR_OK, out);
                         ...
                         void *keys)
{
    ...
    err = cccbc_init_internal(ctx->cbc_info, cbc_ctx, ctx->key_nbytes, temp);
    cc_require(err == CCERR_OK, out);
    ...
    while (keys_nbytes > 0) {
        ...
        err = cccbc_update_internal(ctx->cbc_info, cbc_ctx, cbc_iv, 1, x, p);
        cc_require(err == CCERR_OK, out);
...
int ccdrbg_df_bc_init_internal(ccdrbg_df_bc_ctx_t *ctx,
                               ...
                               size_t key_nbytes)
{
    ...
    return cccbc_init_internal(cbc_info, ctx->cbc_ctx, key_nbytes, key);
}
                  ...
                  size_t key_nbytes)
{
    ...
    return ccdrbg_df_bc_init_internal(ctx, cbc_info, key_nbytes);
}
