/* Copyright (c) (2015-2022,2024) Apple Inc. All rights reserved.
 *
...
{
    CC_TEST_DISABLE_DIT_ASSERTS // We call _ws variants that expect DIT enabled
...
static void test_ccn_value_barriers(void)
{
    cc_unit x[2], y[2];

    for (unsigned i = 0; i < VB_RANDOM_TESTS; i += 1) {
        ccn_random(2, x, global_test_rng);
        ccn_random(2, y, global_test_rng);

        CC_TEST_DISABLE_DIT_ASSERTS
        is(CCN_UNIT_MASK, cc_eq_mask_cc_units(2, x, x), "cc_eq_mask_cc_units() failed");
        is((ccn_cmp(2, x, y) == 0) ? CCN_UNIT_MASK : 0, cc_eq_mask_cc_units(2, x, y), "cc_eq_mask_cc_units() failed");
        CC_TEST_ENABLE_DIT_ASSERTS

#if CC_DUNIT_SUPPORTED
        cc_dunit dx = ((cc_dunit)x[0] << CCN_UNIT_BITS) | x[1];
        cc_dunit dy = ((cc_dunit)y[0] << CCN_UNIT_BITS) | y[1];
        cc_dunit sm = ((cc_dunit)CCN_UNIT_MASK << CCN_UNIT_BITS) | CCN_UNIT_MASK;

        is((dx < dy) ? sm : 0, cc_smaller_mask_cc_dunit(dx, dy), "cc_smaller_mask_cc_dunit() failed");
#endif
    }
}
...
int ccn_tests(TM_UNUSED int argc, TM_UNUSED char *const *argv)
{
    ...
    num_tests += 2 * VB_RANDOM_TESTS;          // ccn_ value barriers
#if CC_DUNIT_SUPPORTED
    num_tests += 1 * VB_RANDOM_TESTS;          // ccn_ value barriers
#endif
