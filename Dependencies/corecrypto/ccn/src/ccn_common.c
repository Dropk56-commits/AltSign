...
size_t ccn_write_uint_padded_internal(cc_size n, const cc_unit *cc_counted_by(n) s, size_t out_size, uint8_t *cc_sized_by(out_size) out)
{
    ...
    int offset_int = ccn_write_uint_padded_ct_internal(n, s, out_size, out);
    if (offset_int >= 0) {
        // It worked
        offset = (size_t)offset_int;
    } else {
        // Truncation case, execution depends on the position of the MSByte
        ccn_write_uint_internal(n, s, out_size, out);
    }
    return offset;
}
...
size_t ccn_write_uint_padded(cc_size n, const cc_unit *cc_counted_by(n) s, size_t out_size, uint8_t *cc_sized_by(out_size) out)
{
    ...
    return ccn_write_uint_padded_internal(n, s, out_size, out);
}
