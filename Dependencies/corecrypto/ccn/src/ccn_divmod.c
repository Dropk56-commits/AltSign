/* Copyright (c) (2012,2015,2021,2022,2024) Apple Inc. All rights reserved.
 *
...
static cc_unit _bquot(cc_unit d)
{
    ...
    for (size_t i = 0; i <= CCN_UNIT_BITS; i++) {
        cc_dunit ge = ~cc_smaller_mask_cc_dunit(y, dd);

        // if (y >= d) y -= d
        y -= dd & ge;

        // q = (q << 1) | (y >= d)
        q = (q << 1) | cc_value_barrier_cc_unit(ge & 1);
...
static cc_unit compute_v(cc_unit d)
{
    ...
    cc_unit m = cc_nonzero_mask_cc_unit(d ^ (CC_UNIT_C(1) << (CCN_UNIT_BITS - 1)));
...
{
    // add := (a0 < d) ? 1 : 2;
    cc_unit add = 2 + cc_smaller_mask_cc_unit(a0, d);
    ...
    // if (q < a1) q := 2^w-1
    q |= cc_smaller_mask_cc_unit(q, a1);
...
void ccn_divmod_ws(cc_ws_t ws, cc_size na, const cc_unit *a, cc_size nq, cc_unit *q, cc_size n, cc_unit *r, const cc_unit *d)
{
    ...
    // sm := (s > 0) ? 2^w-1 : 0
    cc_unit sm = cc_nonzero_mask_cc_unit(s);
