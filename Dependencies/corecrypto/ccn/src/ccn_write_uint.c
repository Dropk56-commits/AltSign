/* Copyright (c) (2010,2011,2015,2018,2019,2021,2023,2024) Apple Inc. All rights reserved.
 *
...
size_t ccn_write_uint_size_public_value(cc_size n, const cc_unit *s)
{
    return CC_BITLEN_TO_BYTELEN(ccn_bitlen_public_value(n, s));
}

size_t ccn_write_uint_size_internal(cc_size n, const cc_unit *s)
{    
    return ccn_write_uint_size_public_value(n, s);
}
...
size_t ccn_write_uint_size(cc_size n, const cc_unit *s)
{
    ...
    return ccn_write_uint_size_internal(n, s);
}
...
int ccn_write_uint_padded_ct_internal(cc_size n, const cc_unit *s, size_t out_size, uint8_t *to)
{
    int offset = 0;
    cc_size i;
    if (out_size >= CC_INT_MAX || ccn_sizeof_n(n) >= CC_INT_MAX) {
    ...
    if (out_size >= ccn_write_uint_size_internal(n, s)) { // Make sure the full big integer can fit
        offset += (out_size - ccn_write_uint_size_internal(n, s));
        to += out_size;
...
int ccn_write_uint_padded_ct(cc_size n, const cc_unit *s, size_t out_size, uint8_t *to)
{
    ...
    return ccn_write_uint_padded_ct_internal(n, s, out_size, to);
}
...
void ccn_write_uint_public_value(cc_size n, const cc_unit *s, size_t out_size, void *out)
{
    ...
    size_t s_size = ccn_write_uint_size_public_value(n, s);
    if (out_size > s_size) {
        out_size = s_size;
    }
...
void ccn_write_uint_internal(cc_size n, const cc_unit *s, size_t out_size, void *out)
{
    ...
    return ccn_write_uint_public_value(n, s, out_size, out);
}
...
void ccn_write_uint(cc_size n, const cc_unit *s, size_t out_size, void *out)
{
    ...
    return ccn_write_uint_internal(n, s, out_size, out);
}
