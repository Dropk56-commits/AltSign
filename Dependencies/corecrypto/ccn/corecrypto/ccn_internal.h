...
int ccn_cmp_internal(cc_size n, const cc_unit *cc_counted_by(n) s, const cc_unit *cc_counted_by(n) t);
...
#define cc_bit_to_mask_cc_unit cc_bit_to_mask_uint64
#define cc_nonzero_mask_cc_unit cc_nonzero_mask_uint64
#define cc_eq_mask_cc_unit cc_eq_mask_uint64
#define cc_smaller_mask_cc_unit cc_smaller_mask_uint64
#define cc_value_barrier_cc_unit cc_value_barrier_uint64

 #if CC_DUNIT_SUPPORTED
  typedef unsigned cc_dunit __attribute__((mode(TI)));

  CC_INLINE CC_WARN_RESULT
  cc_dunit cc_smaller_mask_cc_dunit(cc_dunit a, cc_dunit b)
  {
      cc_unit lt = (a ^ ((a^b) | ((a-b) ^ a))) >> 127;
      return -(cc_dunit)cc_value_barrier_cc_unit(lt);
  }
...
#define cc_bit_to_mask_cc_unit cc_bit_to_mask_uint32
#define cc_nonzero_mask_cc_unit cc_nonzero_mask_uint32
#define cc_eq_mask_cc_unit cc_eq_mask_uint32
#define cc_smaller_mask_cc_unit cc_smaller_mask_uint32
#define cc_value_barrier_cc_unit cc_value_barrier_uint32

CC_INLINE CC_WARN_RESULT
cc_dunit cc_smaller_mask_cc_dunit(cc_dunit a, cc_dunit b)
{
    cc_unit lt = (a ^ ((a^b) | ((a-b) ^ a))) >> 63;
    return -(cc_dunit)cc_value_barrier_cc_unit(lt);
}
...
#define cc_nonzero_mask_cc_size cc_nonzero_mask_size_t

CC_INLINE CC_WARN_RESULT CC_NONNULL_ALL
cc_unit cc_eq_mask_cc_units(cc_size n, const cc_unit *a, const cc_unit *b)
{
    return ~cc_nonzero_mask_cc_unit((cc_unit)ccn_cmp_internal(n, a, b));
}
...
CC_INLINE cc_unit ccn_select(cc_size start, cc_size end, const cc_unit *cc_counted_by(end) r, cc_size i)
{
    ...
    for (cc_size j = start; j < end; j++) {
        ri |= r[j] & cc_eq_mask_cc_unit((cc_unit)i, (cc_unit)j);
    }
...
/* No speculation barrier version internal versions of core functions */
CC_PURE CC_NONNULL((2, 3))
int ccn_cmp_public_value(cc_size n, const cc_unit *cc_counted_by(n) s, const cc_unit *cc_counted_by(n) t);

CC_NONNULL((2))
size_t ccn_bitlen_internal(cc_size n, const cc_unit *cc_counted_by(n) s);
CC_NONNULL((2))
size_t ccn_bitlen_public_value(cc_size n, const cc_unit *cc_counted_by(n) s);

CC_PURE CC_NONNULL((2)) size_t ccn_write_uint_size_internal(cc_size n, const cc_unit *cc_counted_by(n) s);


CC_NONNULL((2, 4))
int ccn_read_uint_internal(cc_size n, cc_unit *cc_counted_by(n) r, size_t data_nbytes, const uint8_t *cc_sized_by(data_nbytes) data);


CC_NONNULL_ALL
int ccn_cmpn_internal(cc_size ns, const cc_unit *cc_counted_by(ns) s, cc_size nt, const cc_unit *cc_counted_by(nt) t);
CC_NONNULL_ALL
int ccn_cmpn_public_value(cc_size ns, const cc_unit *cc_counted_by(ns) s, cc_size nt, const cc_unit *cc_counted_by(nt) t);

size_t ccn_write_uint_padded_internal(cc_size n, const cc_unit *cc_counted_by(n) s, size_t out_size, uint8_t *cc_sized_by(out_size) out);
int ccn_write_uint_padded_ct_internal(cc_size n, const cc_unit *s, size_t out_size, uint8_t *to);
void ccn_write_uint_internal(cc_size n, const cc_unit *s, size_t out_size, void *out);
void ccn_write_uint_public_value(cc_size n, const cc_unit *s, size_t out_size, void *out);
