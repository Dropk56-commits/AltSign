                                   ...
                                   const void *transcript)
{
    ...
    uint8_t kdk[16];
    err = cccmac_one_shot_generate_internal(ccaes_cbc_encrypt_mode(),
    ...
    err = ccnistkdf_ctr_cmac_internal(ccaes_cbc_encrypt_mode(),
                                      32,
                                      sizeof(kdk), kdk,
...
static int mfi_mac_compute(struct ccsigma_ctx *ctx,
                           ...
                           void *mac)
{
    return cccmac_one_shot_generate_internal(ccaes_cbc_encrypt_mode(),
                                             key_size,
                                             key,
                                             data_size,
                                             data,
                                            ...
                                            void *digest)
{
    ...
    ccdigest_init_internal(digest_info, digest_ctx);

    ccdigest_update_internal(digest_info, digest_ctx, mfi_info->sign_dst_size, mfi_info->sign_dst);
    ...
    ccdigest_update_internal(digest_info, digest_ctx, key_share_size, key_share);
    ...
    err = ccsigma_compute_mac_internal(ctx, key_index, identity_size, identity, tag);
    cc_require(err == CCERR_OK, out);

    ccdigest_update_internal(digest_info, digest_ctx, info->mac.tag_size, tag);
...
static int mfi_aead_seal(struct ccsigma_ctx *ctx,
                         ...
                         void *tag)
{
    return ccccm_one_shot_internal(ccaes_ccm_encrypt_mode(),
                                   key_size,
                                   key,
                                   iv_size,
                                   iv,
                         ...
                         void *tag)
{
    ...
    err = ccccm_one_shot_internal(ccaes_ccm_decrypt_mode(),
                                  key_size,
                                  key,
                                  iv_size,
                                  iv,
    ...
    if (cc_cmp_safe_internal(ctx->info->aead.tag_size, tag, computed_tag)) {
        err = CCERR_INTEGRITY;
    }
