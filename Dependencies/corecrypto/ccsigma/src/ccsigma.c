                 ...
                 struct ccrng_state *rng)
{
    ...
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCEC_GENERATE_KEY_FIPS_WORKSPACE_N(ccec_cp_n(info->key_exchange.curve_params)));
    int rv = ccec_generate_key_fips_ws(ws, info->key_exchange.curve_params, rng, kex_ctx);
                                 ...
                                 struct ccrng_state *rng)
{
    ...
    ccec_const_cp_t cp = ccec_ctx_cp(sign_ctx);
    ...
    *signature_nbytes = sigma_ctx->info->signature.signature_size;
    ...
    err = ccec_sign_composite_ws(ws, sign_ctx, digest_nbytes, digest, r, s, rng);
                               ...
                               const void *signing_key)
{
    ...
    ccec_const_cp_t cp = ctx->info->signature.curve_params;
    cc_size n = ccec_cp_n(cp);
    
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCEC_X963_IMPORT_PRIV_WORKSPACE_N(n));
    int err = ccec_x963_import_priv_ws(ws,
                                       ctx->info->signature.curve_params,
                                       signing_key_size,
                                       signing_key,
                                       sign_ctx);
    CC_FREE_WORKSPACE(ws);
                                ...
                                struct ccrng_state *rng)
{
    ...
    if (shared_secret_size > sizeof(shared_secret)) {
...
int ccsigma_compute_mac_internal(struct ccsigma_ctx *ctx,
                                 size_t key_index,
                                 size_t data_size,
                                 const void *data,
                                 void *tag)
                        ...
                        void *tag)
{
    ...
    return ccsigma_compute_mac_internal(ctx, key_index, data_size, data, tag);
}
                   ...
                   const void *peer_identity)
{
    ...
    ccec_pub_ctx_t verify_ctx = ctx->info->signature.peer_ctx(ctx);
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCEC_VERIFY_COMPOSITE_DIGEST_WORKSPACE_N(ccec_ctx_n(verify_ctx)));
    ...
    err = ccec_verify_composite_digest_ws(ws, verify_ctx, digest_size, digest, r, s, NULL);
    cc_require(err == CCERR_OK, out);
