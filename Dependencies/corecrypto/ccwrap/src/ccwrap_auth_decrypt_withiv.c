/* Copyright (c) (2012,2014-2019,2021,2024) Apple Inc. All rights reserved.
 *
                                        ...
                                        const void *iv)
{
    ...
    *obytes = ccwrap_unwrapped_size(nbytes);
    ...
    cc_require_action(cc_cmp_safe_internal(sizeof(R[0]), &R[0], iv) == 0, out, ret = CCERR_INTEGRITY);
                               ...
                               const void *iv)
{
    ...
    return ccwrap_auth_decrypt_withiv_internal(ecb_mode, ecb_key, nbytes, in, obytes, out, iv);
}
