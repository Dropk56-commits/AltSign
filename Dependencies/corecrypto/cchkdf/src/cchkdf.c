/* Copyright (c) (2014,2015,2017-2022,2024) Apple Inc. All rights reserved.
 *
                            ...
                            void *prk)
{
    ...
    cchmac_internal(di, salt_nbytes, salt, ikm_nbytes, ikm, prk);
    return CCERR_OK;
}
                   ...
                   void *prk)
{
    ...
    return cchkdf_extract_internal(di, salt_nbytes, salt, ikm_nbytes, ikm, prk);
}
                           ...
                           void *dk)
{
    ...
    cchmac_init_internal(di, hci, prk_nbytes, prk);
    ...
    for (size_t i = 1; i <= n; ++i) {
        ...
        cchmac_update_internal(di, hc, Tlen, T);
        cchmac_update_internal(di, hc, info_nbytes, info);
        uint8_t b = (uint8_t)i;
        cchmac_update_internal(di, hc, 1, &b);
        cchmac_final_internal(di, hc, T);
                  ...
                  void *dk)
{
    ...
    return cchkdf_expand_internal(di, prk_nbytes, prk, info_nbytes, info, dk_nbytes, dk);
}
                    ...
                    void *dk)
{
    ...
    int result = cchkdf_extract_internal(di, salt_nbytes, salt, ikm_nbytes, ikm, prk);
    if (result == CCERR_OK) {
        result = cchkdf_expand_internal(di, di->output_size, prk, info_nbytes, info, dk_nbytes, dk);
    }
           ...
           void *dk)
{
    ...
    return cchkdf_internal(di, ikm_nbytes, ikm, salt_nbytes, salt, info_nbytes, info, dk_nbytes, dk);
}
