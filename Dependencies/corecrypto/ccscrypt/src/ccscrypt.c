/* Copyright (c) (2018-2021,2024) Apple Inc. All rights reserved.
 *
         ...
         uint8_t *storage, uint64_t N_in, uint32_t r_in, uint32_t p_in, size_t dk_len, uint8_t *dk)
{
    ...
    if (0 != ccpbkdf2_hmac_internal(ccsha256_di(), password_len, password, salt_len, salt, 1, B_len, B)) {
    ...
    if (0 != ccpbkdf2_hmac_internal(ccsha256_di(), password_len, password, B_len, B, 1, dk_len, dk)) {
        return CCERR_INTERNAL;
    }
