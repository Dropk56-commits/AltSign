/* Copyright (c) (2010-2016,2018,2019,2021,2022,2024) Apple Inc. All rights reserved.
 *
   ...
   void *data)
{
    ...
    cchmac_num(di, hc)=0;
    ccdigest_update_internal(di, cchmac_digest_ctx(di, hc), saltLen, salt);

    cc_store32_be((uint32_t)blockNumber, (uint8_t *) &bn);
    ccdigest_update_internal(di, cchmac_digest_ctx(di, hc), 4, &bn);

    /* Caculate U1 (result goes to outBlock) and copy it to resultBlockPtr. */
	cchmac_final_internal(di,  hc, inBlock);
...
int ccpbkdf2_hmac_internal(const struct ccdigest_info *di,
                           ...
                           size_t dkLen, void *dk)
{
    ...
    cchmac_init_internal(di, hc, passwordLen, password);
                      ...
                      size_t dkLen, void *dk)
{
    ...
    return ccpbkdf2_hmac_internal(di, passwordLen, password, saltLen, salt, iterations, dkLen, dk);
}
