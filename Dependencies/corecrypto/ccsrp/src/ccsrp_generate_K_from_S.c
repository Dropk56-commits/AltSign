/* Copyright (c) (2015-2019,2021,2024) Apple Inc. All rights reserved.
 *
                                            ...
                                            uint8_t *dest)
{
    ...
    size_t T_len = ccn_write_uint_size_internal(n, s);
    ccn_write_uint_internal(n, s, T_len, T);
    ...
    ccdigest_internal(di, T_len / 2, E, G); //  G = SHA(E)
    ccdigest_internal(di, T_len / 2, F, H); //  H = SHA(F)
...
static int ccsrp_mgf_ws(cc_ws_t ws, ccsrp_ctx_t srp, const cc_unit *s, void *dest)
{
    ...
    int rv = ccmgf_internal(di, 2 * di->output_size, dest, ccsrp_ctx_sizeof_n(srp) - offset, buf + offset);
