/* Copyright (c) (2012,2014-2019,2021,2022,2024) Apple Inc. All rights reserved.
 *
...
static int ccsrp_server_generate_public_key_internal(ccsrp_ctx_t srp,
                                                     struct ccrng_state *rng,
                                                     const void *verifier,
                                                     void *B_bytes)
{
                                     ...
                                     void *B_bytes)
{
    ...
    return ccsrp_server_generate_public_key_internal(srp, rng, verifier, B_bytes);
}
...
static int ccsrp_server_compute_session_internal(ccsrp_ctx_t srp,
                                                 const char *username,
                                                 size_t salt_len,
                                                 const void *salt,
                                                 const void *A_bytes)
                                 ...
                                 const void *A_bytes)
{
    ...
    return ccsrp_server_compute_session_internal(srp, username, salt_len, salt, A_bytes);
}
                                      ...
                                      void *B_bytes)
{
    ...
    cc_require((status = ccsrp_server_generate_public_key_internal(srp, rng, verifier, B_bytes)) == 0,
               errOut);
    ...
    cc_require((status = ccsrp_server_compute_session_internal(srp, username, salt_len, salt, A_bytes)) == 0,
               errOut);
...
bool ccsrp_server_verify_session(ccsrp_ctx_t srp, const void *user_M, void *HAMK_bytes)
{
    ...
    int cmp = cc_cmp_safe_internal(ccsrp_ctx_M_HAMK_size(srp), ccsrp_ctx_M(srp), user_M);
