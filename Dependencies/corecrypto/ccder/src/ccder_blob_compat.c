                                     ...
                                     uint8_t *der_end)
{
    ccder_blob blob;
    CCDER_BLOB_INIT(blob, der, der_end);
    return ccder_blob_encode_eckey_internal(&blob, priv_size, priv_key, oid, pub_size, pub_key) ? blob.der_end : NULL;
}
                            ...
                            uint8_t *der_end)
{
    ...
    return ccder_encode_eckey_internal(priv_size, priv_key, oid, pub_size, pub_key, der, der_end);
}
                                           ...
                                           const uint8_t *der_end)
{
    ...
    return ccder_blob_decode_eckey_internal(&from, version, priv_size, priv_key, oid, &pub_byte_size, pub_key, pub_bit_count) ? from.der : NULL;
}
                                  ...
                                  const uint8_t *der_end)
{
    ...
    return ccder_decode_eckey_internal(version, priv_size, priv_key, oid, pub_bit_count, pub_key, der, der_end);
}
