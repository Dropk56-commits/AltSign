...
int ccdh_pairwise_consistency_check_ws(cc_ws_t ws,
                                       ccdh_const_gp_t gp,
                                       ...
                                       ccdh_full_ctx_t key)
{
    ...
    int rc = CCDH_GENERATE_KEY_CONSISTENCY;
    ...
    size_t ss2_nbytes = ccn_write_uint_size_internal(n, t);
    ccn_write_uint_padded_internal(n, t, ss2_nbytes, ss2);

    bool ok = (ss1_nbytes == ss2_nbytes) && (cc_cmp_safe_internal(ss1_nbytes, ss1, ss2) == 0);
    if (ok) {
        rc = CCERR_OK;
    }
