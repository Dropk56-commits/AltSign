...
static uint8_t compare_prime(cc_size np, const cc_unit *p, ccdh_const_gp_t group)
{
    ...
    return (uint8_t)ccn_cmpn_public_value(num_length, p, CCDH_GP_N(group), CCDH_GP_PRIME(group));
}
...
static uint8_t compare_generator(cc_size np, const cc_unit *p, ccdh_const_gp_t group)
{
    ...
    return (uint8_t)ccn_cmpn_public_value(num_length, p, g_length, CCDH_GP_G(group));
}
