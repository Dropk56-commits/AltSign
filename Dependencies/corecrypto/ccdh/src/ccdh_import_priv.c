/* Copyright (c) (2011,2015-2022,2024) Apple Inc. All rights reserved.
 *
                               ...
                               ccdh_full_ctx_t key)
{
    ...
    int rv = ccn_read_uint_internal(n, x, in_len, in);
    if (rv) {
        rv = CCDH_INVALID_INPUT;
        goto cleanup;
    }

    if (ccn_cmp_internal(n, x, cczp_prime(ccdh_gp_zp(gp))) >= 0) {
