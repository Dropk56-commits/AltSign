                                      ...
                                      size_t l_bits)
{
    ...
    int rv = ccn_read_uint_internal(n, CCDH_GP_PRIME(gp), p_nbytes, p);
    ...
    CCDH_GP_N(gp) = n;
    rv = ccn_read_uint_internal(n, CCDH_GP_G(gp), g_nbytes, g);
    cc_require_or_return(rv == CCERR_OK, rv);
    ...
    if (q) {
        rv = ccn_read_uint_internal(n, CCDH_GP_Q(gp), q_nbytes, q);
