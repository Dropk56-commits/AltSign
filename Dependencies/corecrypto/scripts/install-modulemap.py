...
corecrypto_submodules = {
	...
	"spake2":        ["ccspake.h"],
	"secretsharing": ["ccss_shamir.h", "ccss.h"],
	"apsic":         ["ccapsic.h"],
	"rsabssa":       ["ccrsabssa.h"],
...
def generate_module_map(module_name):
	module_map = ModuleMap(module_name, attributes = [Attribute.system])
...
if __name__ == "__main__":
	parser = ArgumentParser(prog="install-modulemap.py",
		description="Generate a module map based on headers published during the build")
	parser.add_argument("module_name") # A hack until SEPOS is able to fix its header includes
	args = parser.parse_args()

	# Generate the map
	module_map = generate_module_map(args.module_name)
