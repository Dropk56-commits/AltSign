...
#include "ccchacha20poly1305_internal.h"
...
static int ccchacha20_init_internal(ccchacha20_ctx *ctx, const uint8_t *key)
{
...
int	ccchacha20_init(ccchacha20_ctx *ctx, const uint8_t *key)
{
    ...
    return ccchacha20_init_internal(ctx, key);
}
...
static int ccchacha20_setnonce_internal(ccchacha20_ctx *ctx, const uint8_t *nonce)
{
    CC_ASSERT_DIT_IS_ENABLED
...
int	ccchacha20_setnonce(ccchacha20_ctx *ctx, const uint8_t *nonce)
{
    ...
    return ccchacha20_setnonce_internal(ctx, nonce);
}

static int ccchacha20_setcounter_internal(ccchacha20_ctx *ctx, uint32_t counter)
{
    CC_ASSERT_DIT_IS_ENABLED
...
int	ccchacha20_setcounter(ccchacha20_ctx *ctx, uint32_t counter)
{
    ...
    return ccchacha20_setcounter_internal(ctx, counter);
}
...
static int ccchacha20_update_internal(ccchacha20_ctx *ctx, size_t nbytes, const void *in, void *out )
{
    CC_ASSERT_DIT_IS_ENABLED
...
int	ccchacha20_update( ccchacha20_ctx *ctx, size_t nbytes, const void *in, void *out )
{
    ...
    return ccchacha20_update_internal(ctx, nbytes, in, out);
}

static int ccchacha20_final_internal(ccchacha20_ctx *ctx)
{
    CC_ASSERT_DIT_IS_ENABLED
...
int	ccchacha20_final(ccchacha20_ctx *ctx)
{
    ...
    return ccchacha20_final_internal(ctx);
}
...
int ccchacha20(const uint8_t *key, const uint8_t *nonce, uint32_t counter, size_t nbytes, const void *in, void *out)
{
    ...
    ccchacha20_init_internal(&ctx, key);
    ccchacha20_setnonce_internal(&ctx, nonce);
    ccchacha20_setcounter_internal(&ctx, counter);
    _ccchacha20_xor(&ctx, nbytes, out, in);
    ccchacha20_final_internal(&ctx);
...
static int ccpoly1305_init_internal(ccpoly1305_ctx *ctx, const uint8_t *key)
{
    CC_ASSERT_DIT_IS_ENABLED
...
int ccpoly1305_init(ccpoly1305_ctx *ctx, const uint8_t *key)
{
    ...
    return ccpoly1305_init_internal(ctx, key);
}

static int ccpoly1305_update_internal(ccpoly1305_ctx *ctx, size_t nbytes, const uint8_t *in)
{
    CC_ASSERT_DIT_IS_ENABLED
...
int ccpoly1305_update(ccpoly1305_ctx *ctx, size_t nbytes, const uint8_t *in)
{
    ...
    return ccpoly1305_update_internal(ctx, nbytes, in);
}

static int ccpoly1305_final_internal(ccpoly1305_ctx *ctx, uint8_t *tag)
{
    CC_ASSERT_DIT_IS_ENABLED
...
int ccpoly1305_final(ccpoly1305_ctx *ctx, uint8_t *tag)
{
    ...
    return ccpoly1305_final_internal(ctx, tag);
}
...
int	ccpoly1305(const uint8_t *key, size_t nbytes, const uint8_t *data, uint8_t *out)
{
	...
	ccpoly1305_init_internal(&ctx, key);
	ccpoly1305_update_internal(&ctx, nbytes, data);
	ccpoly1305_final_internal(&ctx, out);
...
static int ccchacha20poly1305_init_internal(CC_UNUSED const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, const uint8_t *key)
{
    CC_ASSERT_DIT_IS_ENABLED

    ccchacha20_init_internal(&ctx->chacha20_ctx, key);
...
int	ccchacha20poly1305_init(const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, const uint8_t *key)
{
    ...
    return ccchacha20poly1305_init_internal(info, ctx, key);
}
...
static int ccchacha20poly1305_setnonce_internal(CC_UNUSED const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, const uint8_t *nonce)
{
    CC_ASSERT_DIT_IS_ENABLED
    ...
    ccchacha20_setnonce_internal(&ctx->chacha20_ctx, nonce);
    _ccchacha20_xor(&ctx->chacha20_ctx, sizeof (kZero64), block, kZero64);
    ccpoly1305_init_internal(&ctx->poly1305_ctx, block);
...
int ccchacha20poly1305_setnonce(const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, const uint8_t *nonce)
{
    ...
    return ccchacha20poly1305_setnonce_internal(info, ctx, nonce);
}
...
static int ccchacha20poly1305_aad_internal(CC_UNUSED const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, size_t nbytes, const void *aad)
{
    CC_ASSERT_DIT_IS_ENABLED

    cc_require(ctx->state == CCCHACHA20POLY1305_STATE_AAD, err);

    ccpoly1305_update_internal(&ctx->poly1305_ctx, nbytes, aad);
...
int	ccchacha20poly1305_aad(const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, size_t nbytes, const void *aad)
{
    ...
    return ccchacha20poly1305_aad_internal(info, ctx, nbytes, aad);
}
...
static int finalize_aad(CC_UNUSED const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, uint8_t state)
{
    if (ctx->state == CCCHACHA20POLY1305_STATE_AAD) {
        ccpoly1305_update_internal(&ctx->poly1305_ctx, pad_nbytes(ctx->aad_nbytes), kZero64);
        ctx->state = state;
    }
...
static int ccchacha20poly1305_encrypt_internal(CC_UNUSED const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, size_t nbytes, const void *in, void *out)
{
    CC_ASSERT_DIT_IS_ENABLED
    ...
    ccchacha20_update_internal(&ctx->chacha20_ctx, nbytes, in, out);
    ccpoly1305_update_internal(&ctx->poly1305_ctx, nbytes, out);
...
int	ccchacha20poly1305_encrypt(const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, size_t nbytes, const void *in, void *out)
{
    CC_ENSURE_DIT_ENABLED_WITH_SB

    return ccchacha20poly1305_encrypt_internal(info, ctx, nbytes, in, out);
}

static int ccchacha20poly1305_decrypt_internal(CC_UNUSED const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, size_t nbytes, const void *in, void *out)
{
    CC_ASSERT_DIT_IS_ENABLED
    ...
    ccpoly1305_update_internal(&ctx->poly1305_ctx, nbytes, in);
    ccchacha20_update_internal(&ctx->chacha20_ctx, nbytes, in, out);
    ctx->text_nbytes += nbytes;
...
int	ccchacha20poly1305_decrypt(const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, size_t nbytes, const void *in, void *out)
{
    CC_ENSURE_DIT_ENABLED_WITH_SB

    return ccchacha20poly1305_decrypt_internal(info, ctx, nbytes, in, out);
}

static int ccchacha20poly1305_finalize_internal(CC_UNUSED const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, uint8_t *tag)
{
    CC_ASSERT_DIT_IS_ENABLED
    ...
    ccpoly1305_update_internal(&ctx->poly1305_ctx, pad_nbytes(ctx->text_nbytes), kZero64);

    cc_store64_le(ctx->aad_nbytes, buf);
    ccpoly1305_update_internal(&ctx->poly1305_ctx, sizeof (uint64_t), buf);
    cc_store64_le(ctx->text_nbytes, buf);
    ccpoly1305_update_internal(&ctx->poly1305_ctx, sizeof (uint64_t), buf);

    ccpoly1305_final_internal(&ctx->poly1305_ctx, tag);
...
int	ccchacha20poly1305_finalize(const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, uint8_t *tag)
{
    ...
    return ccchacha20poly1305_finalize_internal(info, ctx, tag);
}

static int ccchacha20poly1305_verify_internal(CC_UNUSED const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, const uint8_t *tag)
{
    CC_ASSERT_DIT_IS_ENABLED
    ...
    ccpoly1305_final_internal(&ctx->poly1305_ctx, calc_tag);

    ctx->state = CCCHACHA20POLY1305_STATE_FINAL;

	return ( cc_cmp_safe_internal( 16, calc_tag, tag ) == 0 ) ? 0 : -1;
...
int	ccchacha20poly1305_verify(const struct ccchacha20poly1305_info *info, ccchacha20poly1305_ctx *ctx, const uint8_t *tag)
{
    ...
    return ccchacha20poly1305_verify_internal(info, ctx, tag);
}
...
int ccchacha20poly1305_encrypt_oneshot(const struct ccchacha20poly1305_info *info, const uint8_t *key, const uint8_t *nonce, size_t aad_nbytes, const void *aad, size_t ptext_nbytes, const void *ptext, void *ctext, uint8_t *tag)
{
	...
	ccchacha20poly1305_init_internal(info, &ctx, key);
    ccchacha20poly1305_setnonce_internal(info, &ctx, nonce);
    ccchacha20poly1305_aad_internal(info, &ctx, aad_nbytes, aad);
    ccchacha20poly1305_encrypt_internal(info, &ctx, ptext_nbytes, ptext, ctext);
    ccchacha20poly1305_finalize_internal(info, &ctx, tag);
    ccchacha20_final_internal(&ctx.chacha20_ctx);
    return 0;
}
...
int ccchacha20poly1305_decrypt_oneshot(const struct ccchacha20poly1305_info *info, const uint8_t *key, const uint8_t *nonce, size_t aad_nbytes, const void *aad, size_t ctext_nbytes, const void *ctext, void *ptext, const uint8_t *tag)
{
    ...
    ccchacha20poly1305_setnonce_internal(info, &ctx, nonce);
    ccchacha20poly1305_aad_internal(info, &ctx, aad_nbytes, aad);
    ccchacha20poly1305_decrypt_internal(info, &ctx, ctext_nbytes, ctext, ptext);
    int rv = ccchacha20poly1305_verify_internal(info, &ctx, tag);
    ccchacha20_final_internal(&ctx.chacha20_ctx);
    return rv;
}
