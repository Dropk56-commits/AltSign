/* Copyright (c) (2012-2015,2017-2019,2021,2022,2024) Apple Inc. All rights reserved.
 *
...
size_t ccz_write_radix_size(const ccz *s, unsigned radix)
{
    ...
    if (radix == 16) {
        return cc_ceiling(ccn_bitlen_internal(n, s->u), 4) + sign;
    }
...
{
    size_t nbits = ccn_bitlen_internal(ccz_n(s), s->u);
