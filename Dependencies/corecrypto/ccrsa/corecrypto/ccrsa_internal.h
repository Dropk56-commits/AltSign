...
/*!
 @function   ccrsa_sign_pkcs1v15_msg_blinded_ws
 @abstract   Same as ccrsa_sign_pkcs1v15_msg_blinded, with explicit workspace
 ...
 */
CC_NONNULL((1, 2, 3, 4, 6, 7, 8))
int ccrsa_sign_pkcs1v15_msg_blinded_ws(cc_ws_t ws,
                                       struct ccrng_state *blinding_rng,
...
/*!
  @function   ccrsa_verify_pkcs1v15_msg_ws
...
int ccrsa_verify_pkcs1v15_msg_ws(cc_ws_t ws,
                                 ccrsa_pub_ctx_t key,
                                 const struct ccdigest_info* di,
                                 size_t msg_len, const uint8_t *cc_counted_by(msg_len) msg,
...
CC_NONNULL((2, 4))
int ccrsa_emsa_pkcs1v15_encode_internal(size_t emlen, uint8_t *cc_counted_by(emlen) em,
                                        size_t dgstlen, const uint8_t *cc_counted_by(dgstlen) dgst,
                                        const uint8_t *cc_unsafe_indexable oid);

CC_NONNULL((1, 2, 4, 6, 8))
int ccrsa_emsa_pss_encode_internal(const struct ccdigest_info* di, const struct ccdigest_info* MgfDi,
...
int ccrsa_eme_pkcs1v15_encode_internal(struct ccrng_state *rng,
                                       size_t r_size, cc_unit *r,
                                       size_t s_size, const uint8_t *s);

CC_NONNULL((1, 2, 4, 6))
int ccrsa_oaep_encode_parameter_internal(const struct ccdigest_info* di,
                                         struct ccrng_state *rng,
...
CC_NONNULL((1, 2, 3))
uint8_t *ccder_encode_rsa_priv_internal(const ccrsa_full_ctx_t key, const uint8_t *cc_ended_by(der_end) der, uint8_t *der_end);

CC_NONNULL((1, 2, 3))
const uint8_t *ccder_decode_rsa_priv_internal(const ccrsa_full_ctx_t key, const uint8_t *cc_ended_by(der_end) der, const uint8_t *der_end);
