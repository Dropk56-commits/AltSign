/* Copyright (c) (2012,2014-2017,2019-2022,2024) Apple Inc. All rights reserved.
 *
...
static int ccrsa_ensure_2p_gt_q_ws(cc_ws_t ws, cc_size np, const cc_unit *p, cc_size nq, const cc_unit *q)
{
    ...
    int rv = ccn_cmpn_internal(np + 1, tmp, nq, q);

    // Return 0 (if 2*p > q) or -1.
    rv = ((rv + 1) >> 1) - 1;
...
const uint8_t *ccder_decode_rsa_priv_ws(cc_ws_t ws, const ccrsa_full_ctx_t key, const uint8_t *der, const uint8_t *der_end)
{
    ...
    cc_require_action(cczp_mm_init_ws(ws, ccrsa_ctx_zm(key), n, NULL) == CCERR_OK, errOut, der = NULL);

    cczp_t zp = ccrsa_ctx_private_zp(key);
    der = ccder_decode_uint(pqn, CCZP_PRIME(zp), der, der_end);
    cc_require(der != NULL, errOut);

    CCZP_N(zp) = ccn_nof(ccn_bitlen_internal(pqn, cczp_prime(zp)));
    ...
    CCZP_N(zq) = ccn_nof(ccn_bitlen_internal(pqn, cczp_prime(zq)));
...
const uint8_t *ccder_decode_rsa_priv_internal(const ccrsa_full_ctx_t key, const uint8_t *der, const uint8_t *der_end)
{
    CC_ASSERT_DIT_IS_ENABLED
...
const uint8_t *ccder_decode_rsa_priv(const ccrsa_full_ctx_t key, const uint8_t *der, const uint8_t *der_end)
{
    ...
    return ccder_decode_rsa_priv_internal(key, der, der_end);
}
