                                        ...
                                        const cc_unit *Xq)
{
    ...
    size_t pbits = ccn_bitlen_internal(n, p);
    ...
    int r1 = ccn_cmp_internal(n, tmp, delta);

    // Abs(Xp,Xq)
    (void)ccn_abs(n, tmp, Xp, Xq);
    int r2 = ccn_cmp_internal(n, tmp, delta);
                           ...
                           struct ccrng_state *rng)
{
    ...
    for (size_t i = 0; i < SEED_X_MAX_RETRIES; i += 1) {
        ...
        if (ccn_cmp_internal(SQRT2_N, tmp, SQRT2) >= 0) {
            status = CCERR_OK;
            break;
        }
    }
                                              ...
                                              struct ccrsa_fips186_trace *trace)
{
    ...
    if (trace) {
        ...
        trace->bitlen1 = ccn_bitlen_internal(n, r1);
        trace->bitlen2 = ccn_bitlen_internal(n, r2);
                                                            ...
                                                            struct ccrsa_fips186_trace *trace)
{
    ...
    R[0]=pbits;
    r1r2x2max_bitsize=pbits-ccn_bitlen_internal(1,R)-5;
    ...
    } else if (ccn_bitlen_internal(n, r1r2x2) > r1r2x2max_bitsize) {
    ...
    for (size_t ctr=0; (ctr<RESEED_MAX_RETRIES) && (prime_status==CCRSA_KEYGEN_PRIME_NEED_NEW_SEED);ctr++)
    {
        ...
        {
            // 6) Check p >= 2^pbits
            if ((c > 0) || (pbits < ccn_bitlen_internal(n, p))) {
                                         ...
                                         const cc_unit *q)
{
    ...
    if (ccn_bitlen_internal(n_pq, p) + ccn_bitlen_internal(n_pq, q) > ccn_bitsof_n(n)) {
    ...
    if (ccn_cmp_internal(n_pq, p, q) < 0) {
        const cc_unit *t = p;
        p = q;
        q = t;
    }
                                                   ...
                                                   struct ccrsa_fips186_trace *trace)
{
    ...
    size_t ebitlen = ccn_bitlen_internal(ne, e);
                                               ...
                                               struct ccrsa_fips186_trace *trace)
{
    ...
    if (ccn_read_uint_internal(n, ccrsa_ctx_e(pubk), e_nbytes, e_bytes)) {
        return CCRSA_KEY_ERROR;
    }
    ...
    } while (ccn_cmpn_internal(n, d, n_pq, dlb) < 1);
                              ...
                              cc_unit *r_d)
{
    ...
    // e must be > 2 and odd.
    cc_require_action((e[0] & 1) && ccn_bitlen_internal(ne, e) > 1, errOut, ret = CCRSA_KEY_ERROR);

    // Generate P
    ccrng_rsafips_test_init(&rng,xp1Len,xp1,xp2Len,xp2,xpLen,xp);
    x1_bitsize = ccn_bitlen_internal(xp1Len, xp1);
    x2_bitsize = ccn_bitlen_internal(xp2Len, xp2);
    ...
    x1_bitsize = ccn_bitlen_internal(xq1Len, xq1);
    x2_bitsize = ccn_bitlen_internal(xq2Len, xq2);
