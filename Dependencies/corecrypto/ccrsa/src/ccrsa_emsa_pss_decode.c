/* Copyright (c) (2015,2016,2019-2022,2024) Apple Inc. All rights reserved.
 *
                                        ...
                                        cc_fault_canary_t fault_canary_out)
{
    ...
    rc |= ccmgf_internal(MgfDi, len, dbMask, hSize, H);
    ...
    ccdigest_update_internal(di, dc, mSize, mHash);
    if (sSize > 0) {
        ccdigest_update_internal(di, dc, sSize, salt);
    }
    ccdigest_final(di, dc, H2);
    ccdigest_di_clear(di, dc);

    // 14.
    rc |= cc_cmp_safe_internal(hSize, H2, H);
    rc = CCRSA_DECODING_ERROR & cc_nonzero_mask_int(rc);
