/* Copyright (c) (2015,2016,2018-2022,2024) Apple Inc. All rights reserved.
 *
                                     ...
                                     size_t *sigSize, uint8_t *sig)
{
    ...
    *sigSize=modBytes;
    ...
    rc|=ccrsa_emsa_pss_encode_internal(hashAlgorithm, MgfHashAlgorithm, saltSize, salt, hSize, mHash, emBits, (uint8_t *)EM+ofs);     //let it continue, although we know there might be an error
    ...
    if (rc == CCERR_OK && rc_rng == CCERR_OK) {
        ccn_write_uint_padded_ct_internal(modWords, EM, *sigSize, sig);
    } else {
