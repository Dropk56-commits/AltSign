/* Copyright (c) (2020-2022,2024) Apple Inc. All rights reserved.
 *
                                      ...
                                      struct ccrng_state *rng)
{
    ...
    ccdigest_internal(di, msg_nbytes, msg, msg_hash);
    ...
    rc = ccrsa_emsa_pss_encode_internal(di, di, ciphersuite->salt_size_nbytes, salt, di->output_size, msg_hash, emBits, (uint8_t *)EM+ofs);
    ...
    rc = cczp_generate_non_zero_element_ws(ws, zm, rng, r);
    cc_require(rc == CCERR_OK, errOut);

    cc_unit *r_inv = CC_ALLOC_WS(ws, n);
    rc = cczp_inv_ws(ws, zm, r_inv, r);
    cc_require(rc == CCERR_OK, errOut);

    cczp_from_ws(ws, zm, r, r);
    cczp_from_ws(ws, zm, r_inv, r_inv);
    ...
    cczp_mul_ws(ws, zm, z, EM, X);

    cczp_to_ws(ws, zm, z, z);
    rc = ccn_write_uint_padded_ct_internal(n, z, modulus_n_bytes, blinded_msg);
    cc_require(rc >= 0, errOut);

    rc = ccn_write_uint_padded_ct_internal(n, r_inv, modulus_n_bytes, blinding_inverse);
    cc_require(rc >= 0, errOut);
                                          ...
                                          const size_t unblinded_signature_nbytes)
{
    ...
    cc_unit *z = CC_ALLOC_WS(ws, n);
    rc = ccn_read_uint_internal(n, z, blind_signature_nbytes, blind_signature);
    cc_require(rc == CCERR_OK, errOut);

    cc_unit *blind_inverse = CC_ALLOC_WS(ws, n);
    rc = ccn_read_uint_internal(n, blind_inverse, modulus_n_bytes, blinding_inverse);
    cc_require(rc == CCERR_OK, errOut);

    cc_unit *s = CC_ALLOC_WS(ws, n);
    cczp_mul_ws(ws, zm, s, z, blind_inverse);
    cczp_to_ws(ws, zm, s, s);

    rc = ccn_write_uint_padded_ct_internal(n, s, modulus_n_bytes, unblinded_signature);
    cc_require(rc >= 0, errOut);
                                             ...
                                             struct ccrng_state *blinding_rng)
{
    ...
    int rc = ccn_read_uint_internal(n, blindedMessage, blinded_message_nbytes, blinded_message);
    cc_require(rc == CCERR_OK, errOut);

    // Require that encoded blindedMessage < n
    cc_require_action(ccn_cmp_internal(n, key->pb_ccn, blindedMessage) == 1, errOut, rc = CCERR_PARAMETER);
    
    rc = ccrsa_priv_crypt_blinded_ws(ws, blinding_rng, key, signatureCCN, blindedMessage);
    cc_require(rc == CCERR_OK, errOut);

    rc = ccn_write_uint_padded_ct_internal(n, signatureCCN, signature_nbytes, signature);
    cc_require(rc >= 0, errOut);
