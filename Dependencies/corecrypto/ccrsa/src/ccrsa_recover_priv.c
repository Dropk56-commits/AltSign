                                        ...
                                        cc_unit *y, struct ccrng_state *rng)
{
    ...
    nm1[0] &= ~CC_UNIT_C(1);
    cczp_to_ws(ws, zm, nm1, nm1);

    ccn_seti(n, one, 1);
    cczp_to_ws(ws, zm, one, one);
    ...
    for (unsigned i = 0; i < RECOVER_MAX_TRIES; i++) {
        ...
        // y = g^r (mod n)
        rv = cczp_power_ws(ws, zm, y, g, r_bitlen, r);
        if (rv) {
            goto cleanup;
        }

        // if y=1 or y=n-1, try next candidate
        if (ccn_cmp_internal(n, y, one) == 0 || ccn_cmp_internal(n, nm1, y) == 0) {
            continue;
        }
        ...
        for (unsigned j = 0; j < t; j++) {
            // x = y^2 (mod n)
            cczp_sqr_ws(ws, zm, x, y);

            // if x=1, return y
            if (ccn_cmp_internal(n, x, one) == 0) {
                goto cleanup;
            }

            // if x=n-1, try next candidate
            if (ccn_cmp_internal(n, nm1, x) == 0) {
                break;
            }
...
static int ccrsa_recover_pq_ws(cc_ws_t ws, ccrsa_full_ctx_t fk, const cc_unit *y)
{
    ...
    // We require that p > q.
    unsigned int s = (unsigned int)ccn_cmp_internal(n, p, q);
                                 ...
                                 struct ccrng_state *rng)
{
    ...
    if (ccn_read_uint_internal(n, m, m_nbytes, m_bytes)) {
        goto cleanup;
    }
    ...
    if (ccn_read_uint_internal(n, e, e_nbytes, e_bytes)) {
        goto cleanup;
    }

    if (ccn_read_uint_internal(n, d, d_nbytes, d_bytes)) {
        goto cleanup;
    }

    rv = cczp_mm_init_ws(ws, zm, n, m);
    if (rv) {
        goto cleanup;
    }
    ...
    if (ccn_cmp_internal(n, m, cczp_prime(zm))) {
        rv = CCRSA_INVALID_INPUT;
    }

    // Sanity check against the original private exponent.
    if (ccn_cmp_internal(n, d, ccrsa_ctx_d(fk))) {
        rv = CCRSA_INVALID_INPUT;
    }
