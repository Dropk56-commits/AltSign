/* Copyright (c) (2011,2013-2016,2019-2024) Apple Inc. All rights reserved.
 *
                 ...
                 size_t src_size, size_t src_off, const void *src)
{
    ...
    size_t mask = cc_bit_to_mask_size_t(cond & 1);
    ...
    for (size_t i = 0; i < *dst_size; i += 1) {
        ...
        for (size_t j = 0; j < src_size; j += 1) {
            uint8_t b = s[j];
            uint8_t m = (uint8_t)cc_eq_mask_size_t(j, off);
            k ^= (uint8_t)mask & m & b;
        }
    ...
    cc_mux_mask(*dst_size, mask, src_size - src_off, *dst_size);
}
                 ...
                 size_t src0_size, size_t src0_off, const void *src0)
{
    ...
    size_t m = cc_nonzero_mask_size_t((size_t)cond);
    cc_mux_mask(*dst_size, m, tmp1_size, tmp0_size);
...
static int
ccrsa_eme_pkcs1v15_decode_generate_random(size_t entropy_nbytes, const void *entropy,
                                          ...
                                          size_t *rand_offset)
{
    ...
    ccdigest_internal(ccsha256_di(), block_nbytes, block, nonce);
    ...
    int err = ccdrbg_init_internal(&drbg_info, drbg,
    ...
    // We could call `ccdrbg_generate_internal` directly instead of `ccrng_generate`
    ...
    uint64_t rand_off64;
    CC_TEST_DISABLE_NESTED_DIT_CHECKS
    err = ccrng_uniform_internal(rng, rand_nbytes + 1, &rand_off64);
    CC_TEST_ENABLE_NESTED_DIT_CHECKS
    cc_require(err == CCERR_OK, out);
...
static int
ccrsa_eme_pkcs1v15_decode_internal_ws(cc_ws_t ws,
                                      size_t entropy_size, const void *entropy,
                                      ...
                                      size_t s_size, cc_unit *s)
{
    ...
    int retval = ccrsa_eme_pkcs1v15_decode_generate_random(entropy_size, entropy,
                                                           inlen, in,
                                                           rand_size, rand,
                                                           &rand_offset);
    cc_require(retval == CCERR_OK, out);

    uint8_t decoding_ok = 0;

    // Expected structure is
    // 00:02:PS:00:Msg

    // -- Check for expected prefix 00:02
    decoding_ok = cc_eq_mask_uint8(in[0], 0x00);
    decoding_ok &= cc_eq_mask_uint8(in[1], 0x02);

    size_t zero_idx = 0;
    uint8_t looking_for_zero = 0xff;
    ...
    for (size_t i = 2; i < inlen; i++) {
        ...
        size_t m = cc_bit_to_mask_size_t(looking_for_zero & 1);
        cc_mux_mask(zero_idx, m, i, zero_idx);

        looking_for_zero &= cc_nonzero_mask_uint8(in[i]);
    }

    // Fail if we found no 0x00 marker.
    decoding_ok &= ~looking_for_zero;
    ...
    // -- Check that padlen >= 8
    decoding_ok &= cc_nonzero_mask_size_t(padlen >> 3);
 ...
 out:
    ...
    cc_clear(sizeof(rand_offset), &rand_offset);
    cc_clear(sizeof(decoding_ok), &decoding_ok);
...
int ccrsa_eme_pkcs1v15_decode_safe_ws(cc_ws_t ws,
                                      ...
                                      size_t s_size, cc_unit *s)
{
    uint8_t entropy[CCSHA256_OUTPUT_SIZE];

    ccdigest_internal(ccsha256_di(),
                      ccn_sizeof_n(ccrsa_ctx_n(key)),
                      ccrsa_ctx_d(key),
                      entropy);

    int rv = ccrsa_eme_pkcs1v15_decode_internal_ws(ws,
                              ...
                              size_t s_size, cc_unit *s)
{
    ...
    cc_size n = ccrsa_n_from_size(s_size);
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCRSA_EME_PKCS1V15_DECODE_INTERNAL_WORKSPACE_N(n));

    int rv;

    static uint8_t entropy[CCSHA256_OUTPUT_SIZE];
    static bool entropy_init = false;
    if (!entropy_init) {
        struct ccrng_state *rng = ccrng(&rv);
        cc_require(rng != NULL, out);
        rv = ccrng_generate(rng, sizeof(entropy), entropy);
        cc_require(rv == CCERR_OK, out);
        entropy_init = true;
    }

    rv = ccrsa_eme_pkcs1v15_decode_internal_ws(ws,
                                               sizeof(entropy), entropy,
                                               r_size, r,
                                               s_size, s);
