                       ...
                       uint8_t *sig)
{
    ...
    ccdigest_internal(hashAlgorithm, msg_nbytes, msg, digest);
    
    // ccrsa_sign_pss copied here
    struct ccrng_state *blinding_rng = ccrng(NULL);
    if (blinding_rng == NULL) {
        return CCERR_INTERNAL;
    }

    return ccrsa_sign_pss_blinded(blinding_rng, key, hashAlgorithm, MgfHashAlgorithm, salt_nbytes, rng, hashAlgorithm->output_size, digest, sig_nbytes, sig);
}
