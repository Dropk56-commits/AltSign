/* Copyright (c) (2011-2016,2018-2022,2024) Apple Inc. All rights reserved.
 *
                                   ...
                                   const uint8_t *parameter_data)
{
    ...
    ccdigest_internal(di, parameter_data_len, parameter_data, lHash);
    ...
    // Y == 0 ?
    retval = cc_nonzero_mask_int((int)encoded[0]);

    // lHash == lHash' ?
    retval |= cc_cmp_safe_internal(di->output_size, lHash, DB);

    size_t one_idx = 0;
    uint8_t looking_for_one = 0xff;
    ...
    for (size_t i = di->output_size; i < DB_len; i++) {
        ...
        size_t m = cc_bit_to_mask_size_t(looking_for_one & 1);
        cc_mux_mask(one_idx, m, i, one_idx);

        looking_for_one &= ~cc_eq_mask_uint8(ptr[i], 0x01);

        // Fail if there's a non-zero byte before the 0x01 marker.
        retval |= looking_for_one & cc_nonzero_mask_uint8(ptr[i]);
    }
