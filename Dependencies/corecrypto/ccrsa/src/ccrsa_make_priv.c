                                          ...
                                          const uint8_t *q_in)
{
    ...
    cc_require_action(ccn_read_uint_internal(max_pq_blob_n, ccn_p, p_m, p) == 0, error_out, error = CCRSA_INVALID_INPUT);
    cc_require_action(ccn_read_uint_internal(max_pq_blob_n, ccn_q, q_m, q_in) == 0, error_out, error = CCRSA_INVALID_INPUT);
    cc_require_action(ccn_read_uint_internal(max_e_blob_n, ccn_e, e_m, e) == 0, error_out, error = CCRSA_INVALID_INPUT);
    ...
    unsigned int s = (unsigned int)ccn_cmp_internal(max_pq_blob_n, ccn_p, ccn_q);
    ...
    cc_size p_bit_length = ccn_bitlen_internal(max_pq_blob_n, ccn_p);
    cc_size q_bit_length = ccn_bitlen_internal(max_pq_blob_n, ccn_q);
