/* Copyright (c) (2015,2016,2019,2021,2024) Apple Inc. All rights reserved.
 *
                                   ...
                                   size_t emBits, uint8_t *EM)
{
    ...
    ccdigest_update_internal(di, dc, sizeof(uint64_t), &zero);
    ccdigest_update_internal(di, dc, hSize, mHash);
    if(sSize!=0) ccdigest_update_internal(di, dc, sSize, salt); //sLen=0 can be passed to the function, which means no salt
    ...
    rc = ccmgf_internal(MgfDi, emSize-hSize-1, dbMask, hSize, H); //take H and produce dbMask with the length of emLen-hLen-1
                    ...
                    size_t emBits, uint8_t *EM)
{
    ...
    return ccrsa_emsa_pss_encode_internal(di, MgfDi, sSize, salt, hSize, mHash, emBits, EM);
}
