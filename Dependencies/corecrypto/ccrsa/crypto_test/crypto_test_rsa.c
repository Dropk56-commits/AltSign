...
static int crypt_decrypt(ccrsa_full_ctx_t fk)
{
    ...
    CC_TEST_DISABLE_DIT_ASSERTS // Both below functions expect DIT to be enalbled
...
static int pkcs1v15_decode_length_test(ccrsa_full_ctx_t fk, struct ccrng_state *rng)
{
    ...
    for (size_t k = 0; k < (1 << 16); k += 1) {
        ...
        status = ccrsa_eme_pkcs1v15_decode_safe(fk,
...
static int ccrsa_test_setkey(ccrsa_full_ctx_t fk, size_t fk_bit_size, size_t index) {
    ...
    CC_DECL_WORKSPACE_TEST(ws);
    is(cczp_mm_init_ws(ws, ccrsa_ctx_zm(pubk), n, NULL), CCERR_OK, "cczp_mm_init_ws() failed, index=%zu", index);
    CC_FREE_WORKSPACE(ws);
...
test_sample_pkcs1v15(void)
{
    ...
    if ((result_size==expected_sig->len) &&
        (0==memcmp(result,expected_sig->bytes,expected_sig->len))) {
        status|=1<<test_step;
    }
    test_step++;
...
static int test_import_invalid_der_pub_key(void) {
    // Triggers "skip null byte" logic under invalid conditions - should see ASAN crash if conditional fails
    uint8_t badDER[] = {
      0x30, 0x0c, 0x30, 0x0a, 0x06, 0x03, 0xfa, 0xf9, 0xfb, 0x05, 0x00, 0x03, 0x81, 0x00
    };
    unsigned int badDER_len = 14;

    cc_size n = ccrsa_import_pub_n(badDER_len, badDER);
    ccrsa_pub_ctx_decl(ccn_sizeof_n(n), key);
    ccrsa_ctx_n(key) = n;

    int rc = ccrsa_import_pub(key, badDER_len, badDER);
    if (rc == CCERR_OK) {
        printf("No error importing bad pubkey!");
        return 0;
    }
    ccrsa_pub_ctx_clear_n(n, key);
...
int ccrsa_tests(TM_UNUSED int argc, TM_UNUSED char *const *argv)
{
    ...
    ok(test_import_der_pub_key(), "Import valid RSA DER public key");
    ok(test_import_invalid_der_pub_key(), "Import invalid RSA DER public key");
