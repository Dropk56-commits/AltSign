                               ...
                               uint8_t *w_out)
{
    ...
    ccn_write_uint_padded_ct_internal(n, t, w_out_nbytes, w_out);
                                 ...
                                 struct ccrng_state *rng)
{
    ...
    if (scp->var == CCSPAKE_VARIANT_CCC_V1) {
        ...
        // RFC: L := w1 * G
        rv = ccn_read_uint_internal(n, k, w1_nbytes, w1);
        cc_require(rv == CCERR_OK, errOut);
...
int ccspake_cmp_pub_key(ccec_pub_ctx_t pub, const cc_unit *X)
{
    ...
    rv |= ccn_cmp_internal(n, X, ccec_ctx_x(pub));
    rv |= ccn_cmp_internal(n, X + n, ccec_ctx_y(pub));
