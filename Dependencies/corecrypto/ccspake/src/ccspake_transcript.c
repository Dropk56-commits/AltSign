...
{
    ccdigest_init_internal(ccspake_ctx_mac(ctx)->di(), ccspake_ctx_hash(ctx));
}
...
static void ccspake_transcript_append_length(ccspake_ctx_t ctx, uint64_t length)
{
    ...
    ccdigest_update_internal(ccspake_ctx_mac(ctx)->di(), ccspake_ctx_hash(ctx), sizeof(le_length), le_length);
}
...
void ccspake_transcript_append(ccspake_ctx_t ctx, size_t nbytes, const uint8_t *data)
{
    ...
    if (nbytes > 0) {
        ccdigest_update_internal(ccspake_ctx_mac(ctx)->di(), ccspake_ctx_hash(ctx), nbytes, data);
    }
}
...
void ccspake_transcript_append_point(ccspake_ctx_t ctx, ccec_const_cp_t cp, const cc_unit *x, const cc_unit *y)
{
    ...
    ccn_write_uint_padded_internal(n, x, len, xy + 1);
    ccn_write_uint_padded_internal(n, y, len, xy + 1 + len);
...
void ccspake_transcript_append_scalar(ccspake_ctx_t ctx, ccec_const_cp_t cp, const cc_unit *x)
{
    ...
    ccn_write_uint_padded_internal(n, x, len, num);
