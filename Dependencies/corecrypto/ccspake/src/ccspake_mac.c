...
int ccspake_mac_hkdf_derive(ccspake_const_ctx_t ctx, size_t ikm_nbytes, const uint8_t *ikm, uint8_t *keys)
{
    ...
    return cchkdf_internal(di, ikm_nbytes, ikm, 0, NULL, info_nbytes, info, keys_nbytes, keys);
}
                                        ...
                                        uint8_t *t)
{
    ...
    ccn_write_uint_padded_internal(n, x, p_len, info + 1);
    ccn_write_uint_padded_internal(n, y, p_len, info + 1 + p_len);
...
int ccspake_mac_verify_and_get_session_key(ccspake_ctx_t ctx, size_t t_len, const uint8_t *t, size_t sk_len, uint8_t *sk)
{
    ...
    if (cc_cmp_safe_internal(t_len, t, tag)) {
        rv = CCERR_INTEGRITY;
        goto cleanup;
    }
    ...
    if (ccspake_ctx_variant(ctx) == CCSPAKE_VARIANT_CCC_V1) {
        ...
        // RFC: SK = KDF(nil, K_main, "SharedKey")
        rv = cchkdf_internal(di, di->output_size, ccspake_ctx_main_key(ctx), 0, NULL, sizeof(KDF_LABEL_SK), KDF_LABEL_SK, sk_len, sk);
