...
static int ccspake_prover_init_internal(ccspake_ctx_t ctx,
                        ...
                        const uint8_t *w1)
{
    ...
    return ccspake_prover_init_internal(ctx, scp, mac, rng, aad_nbytes, aad, w_nbytes, w0, w1);
}
                              ...
                              const uint8_t *w1)
{
    ...
    if (scp->var == CCSPAKE_VARIANT_CCC_V1) {
        ...
        return ccspake_prover_init_internal(ctx, scp, mac, rng, context_nbytes, context, w_nbytes, w0, w1);
    }
    ...
    // Read w0.
    int rv = ccn_read_uint_internal(n, ccspake_ctx_w0(ctx), w_nbytes, w0);
    cc_require_or_return(rv == CCERR_OK, rv);

    // Read w1.
    rv = ccn_read_uint_internal(n, ccspake_ctx_w1(ctx), w_nbytes, w1);
    cc_require_or_return(rv == CCERR_OK, rv);
                                          ...
                                          const uint8_t *L)
{
    ...
    // Read w0.
    int rv = ccn_read_uint_internal(n, ccspake_ctx_w0(ctx), w0_nbytes, w0);
    cc_require(rv == CCERR_OK, errOut);
