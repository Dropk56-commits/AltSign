/* Copyright (c) (2019-2022,2024) Apple Inc. All rights reserved.
 *
                                  ...
                                  uint8_t *output)
{
    ...
    int ret = cchkdf_extract_internal(params->kdf->hashFunction(), salt_nbytes, salt, labeled_ikm_bytes_written, labeled_ikm, output);
                                 ...
                                 uint8_t *output)
{
    ...
    int ret = cchkdf_expand_internal(params->kdf->hashFunction(), prk_nbytes, prk, labeled_info_bytes_written, labeled_info, L, output);
...
cchpke_kem_x25519_generate_key_pair(struct ccrng_state *rng, size_t sk_nbytes, uint8_t *sk, size_t pk_nbytes, uint8_t *pk)
{
    ...
    int result = cccurve25519_make_key_pair_internal(rng, public_key, secret_key);
    if (result != CCERR_OK) {
        return result;
    }
                                                 ...
                                                 uint8_t *enc)
{
    ...
    int result = cccurve25519_assumes_dit_internal(DH, skE, pkR);
    if (result != CCERR_OK) {
        return result;
    }
                                   ...
                                   uint8_t *key)
{
    ...
    ccec25519pubkey pkR;
    result = cccurve25519_make_pub_internal(pkR, skR);
    if (result != CCERR_OK) {
        return result;
    }
    ...
    ccec25519pubkey DH;
    result = cccurve25519_assumes_dit_internal(DH, skR, pkE);
    if (result != CCERR_OK) {
        return result;
    }
...
static int cchpke_kem_x25519_public_key(size_t sk_nbytes, const uint8_t *sk, size_t pk_nbytes, uint8_t *pk)
{
    ...
    return cccurve25519_make_pub_internal(pk, sk);
}
...
{
    ccdigest_internal(ccsha256_di(), message_nbytes, message, digest);
}
...
static int cchpke_aead_aesgcm128_seal(size_t key_nbytes,
                                      ...
                                      uint8_t *tag)
{
    return ccgcm_one_shot_internal(
...
static int cchpke_initiator_setup_internal(cchpke_initiator_t initiator,
                                           cchpke_const_params_t params,
                                           struct ccrng_state *rng,
                                           size_t pkR_nbytes,
                                           const uint8_t *pkR,
                           ...
                           uint8_t *enc)
{
    ...
    return cchpke_initiator_setup_internal(initiator, params, rng, pkR_nbytes, pkR, info_nbytes, info, enc_nbytes, enc);
}

static int cchpke_responder_setup_internal(cchpke_responder_t responder,
                                           cchpke_const_params_t params,
                                           size_t skR_nbytes,
                                           const uint8_t *skR,
                                           size_t info_nbytes,
                           ...
                           const uint8_t *enc)
{
    ...
    return cchpke_responder_setup_internal(responder, params, skR_nbytes, skR, info_nbytes, info, enc_nbytes, enc);
}
...
static int cchpke_initiator_encrypt_internal(cchpke_initiator_t initiator,
                                             size_t aad_nbytes,
                                             const uint8_t *aad,
                                             size_t pt_nbytes,
                                             const uint8_t *pt,
                             ...
                             uint8_t *tag)
{
    ...
    return cchpke_initiator_encrypt_internal(initiator, aad_nbytes, aad, pt_nbytes, pt, ct, tag_nbytes, tag);
}

static int cchpke_responder_decrypt_internal(cchpke_responder_t responder,
                                             size_t aad_nbytes,
                                             const uint8_t *aad,
                                             size_t ct_nbytes,
                                             const uint8_t *ct,
                             ...
                             uint8_t *pt)
{
    ...
    return cchpke_responder_decrypt_internal(responder, aad_nbytes, aad, ct_nbytes, ct, tag_nbytes, tag, pt);
}
                          ...
                          uint8_t *enc)
{
    ...
    int result = cchpke_initiator_setup_internal(&initiator, params, rng, pkR_nbytes, pkR, info_nbytes, info, enc_nbytes, enc);
    if (result != CCERR_OK) {
        return result;
    }

    result = cchpke_initiator_encrypt_internal(&initiator, aad_nbytes, aad, pt_nbytes, pt, ct, tag_nbytes, tag);
                          ...
                          uint8_t *pt)
{
    ...
    int result = cchpke_responder_setup_internal(&responder, params, skR_nbytes, skR, info_nbytes, info, enc_nbytes, enc);
    if (result != CCERR_OK) {
        return result;
    }

    result = cchpke_responder_decrypt_internal(&responder, aad_nbytes, aad, ct_nbytes, ct, tag_nbytes, tag, pt);
