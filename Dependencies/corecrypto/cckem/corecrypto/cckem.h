...
/*!
 @function cckem_derive_key_from_seed
 @discussion Derives a private and public key pair from a given seed.

 @abstract DO NOT USE unless you are a 100% sure that this is the function you
           want. Most likely you should rather import a full private key to
           ensure the stored key wasn't corrupted.

 @param ctx          A full KEM context.
 @param seed_nbytes  Size of the seed in bytes (must be >= 32 bytes).
 @param seed         Seed to derive keys from.
 @param masking_rng  RNG for masking (a TRNG).

 @return CCERR_OK on success, an error code otherwise.
 */
CC_NONNULL_ALL
int cckem_derive_key_from_seed(cckem_full_ctx_t ctx,
                               size_t seed_nbytes,
                               const uint8_t *cc_sized_by(seed_nbytes) seed,
