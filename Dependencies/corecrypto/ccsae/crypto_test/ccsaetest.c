...
static int ccrng_sae_generate(struct ccrng_state *rng, size_t entropy_size, void *entropy)
{
    ...
    cc_unit s[irng->n];
    if (ccn_read_uint_internal(irng->n, s, scalar->scalar_nbytes, scalar->scalar) != CCERR_OK) {
    ...
    if (ccn_write_uint_padded_internal(irng->n, s, entropy_size, entropy) < 0) {
...
static int ccsae_test_curves_and_hashes(ccec_const_cp_t cp, const struct ccdigest_info *di)
{
    ...
    // Run it twice since we can call the confirmation routine multiple times
    is(ccsae_generate_confirmation(ctxA, A_send_confirmation, A_confirmation), CCERR_OK, "A confirmation generation error");
...
static int ccsae_test_y2_vs_affine_point(ccec_const_cp_t cp)
{
    ...
    CC_TEST_DISABLE_DIT_ASSERTS // ccsae_y2_from_x_ws expects DIT
    y2_res = ccsae_y2_from_x_ws(ws, cp, y, x);
    CC_TEST_ENABLE_DIT_ASSERTS

    is(y2_res, -1, "x = p + 1 should never succeed ccsae_y2_from_x_ws");
    ...
    for (int t = 0; t < y2_vs_ap_test_trials; t++) {
        ...
        CC_TEST_DISABLE_DIT_ASSERTS // These _ws functions expect DIT
        ...
        ok((y2_res == -1 && affine_res != 0) ||
               (y2_res == 0 && affine_res == 0 &&
...
int ccsae_tests(TM_UNUSED int argc, TM_UNUSED char *const *argv)
{
    ...
    num_tests += (31 * (CC_ARRAY_LEN(curves) * CC_ARRAY_LEN(digests))); // # tests of test_curves_and_hashes, # curves, # hashes
