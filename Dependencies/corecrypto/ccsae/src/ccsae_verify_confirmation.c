...
static int ccsae_verify_confirmation_ws(cc_ws_t ws, ccsae_ctx_t ctx, const uint8_t *peer_send_confirm_counter, const uint8_t *peer_confirmation)
{
    ...
    cchmac_init_internal(di, hc, ccsae_sizeof_kck_internal(ctx), ccsae_ctx_KCK(ctx));

    cchmac_update_internal(di, hc, 2, peer_send_confirm_counter);

    ccn_write_uint_padded_internal(n, ccsae_ctx_peer_commitscalar(ctx), tn, scratch);
    cchmac_update_internal(di, hc, tn, scratch);

    ccn_write_uint_padded_internal(n, ccsae_ctx_peer_CE_x(ctx), tn, scratch);
    cchmac_update_internal(di, hc, tn, scratch);

    ccn_write_uint_padded_internal(n, ccsae_ctx_peer_CE_y(ctx), tn, scratch);
    cchmac_update_internal(di, hc, tn, scratch);

    ccn_write_uint_padded_internal(n, ccsae_ctx_commitscalar(ctx), tn, scratch);
    cchmac_update_internal(di, hc, tn, scratch);

    ccn_write_uint_padded_internal(n, ccsae_ctx_CE_x(ctx), tn, scratch);
    cchmac_update_internal(di, hc, tn, scratch);

    ccn_write_uint_padded_internal(n, ccsae_ctx_CE_y(ctx), tn, scratch);
    cchmac_update_internal(di, hc, tn, scratch);

    cchmac_final_internal(di, hc, myconfirm);

    cchmac_di_clear(di, hc);
    CCSAE_ADD_STATE(CONFIRMATION_VERIFIED);

    CC_FREE_BP_WS(ws, bp);
    return cc_cmp_safe_internal(di->output_size, myconfirm, peer_confirmation);
}
