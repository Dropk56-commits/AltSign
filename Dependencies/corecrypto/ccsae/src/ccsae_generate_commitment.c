                                               ...
                                               uint8_t *commitment)
{
    ...
    cc_require(ccn_write_uint_padded_ct_internal(n, ccsae_ctx_commitscalar(ctx), tn, commitment) >= 0, out);
    cc_require(ccn_write_uint_padded_ct_internal(n, ccsae_ctx_CE_x(ctx), tn, commitment + tn) >= 0, out);
    cc_require(ccn_write_uint_padded_ct_internal(n, ccsae_ctx_CE_y(ctx), tn, commitment + 2 * tn) >= 0, out);
...
int ccsae_generate_commitment_init(ccsae_ctx_t ctx)
{
    ...
    ccsae_ctx_found_qr(ctx) = 0xff;
    ccsae_ctx_temp_lsb(ctx) = 0;
                                                ...
                                                uint8_t max_num_iterations)
{
    ...
    ccsae_lexographic_order_key_internal(A, A_nbytes, B, B_nbytes, key);

    CC_DECL_BP_WS(ws, bp);

    // Initialize per-iteration HMAC.
    cchmac_di_decl(di, hc);
    cchmac_init_internal(di, hc, keySize, key);
    cchmac_update_internal(di, hc, password_nbytes, password);
    if (identifier != NULL) {
        cchmac_update_internal(di, hc, identifier_nbytes, identifier);
    }
    ...
    for (uint8_t counter = 0; counter < actual_iterations; counter++) {
        ...
        cchmac_update_internal(di, hc, 1, &actual_counter);
        cchmac_final_internal(di, hc, ccsae_ctx_S_PWD_SEED(ctx));
        ccsae_gen_password_value_ws(ws, ctx, ccsae_ctx_S_PWD_SEED(ctx), ccsae_ctx_S_PWD_VALUE(ctx));

        ccn_mux(n, found_qr & 1, ccsae_ctx_PWE_x(ctx), ccsae_ctx_S_PWD_VALUE(ctx), ccsae_ctx_PWE_x(ctx));
        cc_mux_mask(LSB, found_qr, ccsae_ctx_S_PWD_SEED_LSB(ctx, di) & 1, LSB);

        found_qr &= (uint8_t)ccsae_y2_from_x_ws(ws, cp, ccsae_ctx_PWE_y(ctx), ccsae_ctx_PWE_x(ctx));
    }
...
static int ccsae_generate_commitment_finalize_ws(cc_ws_t ws, ccsae_ctx_t ctx, uint8_t *commitment)
{
    ...
    ccec_const_cp_t cp = ccsae_ctx_cp(ctx);
    cc_size n = ccec_cp_n(cp);
    cc_unit LSB = ccsae_ctx_temp_lsb(ctx);
                              ...
                              uint8_t *commitment)
{
    ...
    ccec_const_cp_t cp = ccsae_ctx_cp(ctx);
    CC_DECL_WORKSPACE_OR_FAIL(ws, CC_MAX_EVAL(CCSAE_GENERATE_COMMITMENT_PARTIAL_WORKSPACE_N(ccec_cp_n(cp)), CCSAE_GENERATE_COMMITMENT_FINALIZE_WORKSPACE_N(ccec_cp_n(cp))));

    int error = ccsae_generate_commitment_init(ctx);
    cc_require(error == CCERR_OK, out);


    error = ccsae_generate_commitment_partial_ws(ws,
        ctx, A, A_nbytes, B, B_nbytes, password, password_nbytes, identifier, identifier_nbytes, SAE_HUNT_AND_PECK_ITERATIONS);
    cc_require(error == CCERR_OK, out);

    error = ccsae_generate_commitment_finalize_ws(ws, ctx, commitment);
out:
    CC_FREE_WORKSPACE(ws);
    return error;
}
                                             ...
                                             size_t pt_nbytes)
{
    ...
    cchmac_internal(di, di->output_size, zeros, keySize, key, hash_bytes);
    error = ccn_read_uint_internal(nd, hash, di->output_size, hash_bytes);
    cc_require(error == CCERR_OK, out);
