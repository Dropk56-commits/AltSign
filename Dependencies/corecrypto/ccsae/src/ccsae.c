...
void ccsae_lexographic_order_key_internal(const uint8_t *A, size_t A_nbytes, const uint8_t *B, size_t B_nbytes, uint8_t *output)
{
...
void ccsae_lexographic_order_key(const uint8_t *A, size_t A_nbytes, const uint8_t *B, size_t B_nbytes, uint8_t *output)
{
    ...
    ccsae_lexographic_order_key_internal(A, A_nbytes, B, B_nbytes, output);
}
...
sae_construct_fixed_data(ccec_const_cp_t cp, const char *label, const cc_unit *context, size_t size, uint8_t *fixedData)
{
    ...
    ccn_write_uint_padded_internal(n, context, tn, fixedData + 2 + label_nbytes);
                                 ...
                                 size_t output_nbytes)
{
    ...
    for (size_t i = 1; i <= iterations; i += 1, output += h, output_nbytes -= h) {
        cchmac_init_internal(di, hc, h, secret);
        fixedData[0] = (uint8_t)i;
        fixedData[1] = (uint8_t)(i >> 8);
        cchmac_update_internal(di, hc, fixedData_nbytes, fixedData);
        cchmac_final_internal(di, hc, result_buf);
...
{    
    ...
    (void)ccn_read_uint_internal(n, output, tn, result_buf);
    if (tnbits % 8 != 0) {
...
int ccsae_y2_from_x_ws(cc_ws_t ws, ccec_const_cp_t cp, cc_unit *y2, const cc_unit *x_in)
{
    CC_ASSERT_DIT_IS_ENABLED
    ...
    // result1 = -1 if x < p, otherwise 0 or 1
    int result1 = ccn_cmp_internal(ccec_cp_n(cp), x, cczp_prime(ccec_cp_zp(cp)));

    // Normalize result: result1 = 0 if x < p, -1 otherwise
    result1 = ~cc_neg_mask_int(result1);

    // Perform the swap when result1 ≠ 0
    ...
    int result2 = cczp_is_quadratic_residue_ws(ws, zp, y2); // 0 on success, -1 otherwise
