...
static void ccsae_generate_keyseed_ws(cc_ws_t ws, ccec_const_cp_t cp, const struct ccdigest_info *di, const struct ccec_projective_point *P, uint8_t *keyseed)
{
    ...
    ccn_write_uint_padded_internal(n, ccec_point_x(P, cp), tn, x_coord);

    cchmac_di_decl(di, hc);
    cchmac_init_internal(di, hc, 32, zeros);
    cchmac_update_internal(di, hc, tn, x_coord);
    cchmac_final_internal(di, hc, keyseed);
...
static int ccsae_verify_commitment_ws(cc_ws_t ws, ccsae_ctx_t ctx, const uint8_t *peer_commitment)
{
    ...
    ccn_read_uint_internal(n, ccsae_ctx_peer_commitscalar(ctx), tn, peer_commitment);
    ccn_read_uint_internal(n, ccsae_ctx_peer_CE_x(ctx), tn, peer_commitment + tn);
    ccn_read_uint_internal(n, ccsae_ctx_peer_CE_y(ctx), tn, peer_commitment + tn + tn);
    ...
    cc_require(ccn_cmp_internal(n, ccsae_ctx_peer_commitscalar(ctx), ccsae_ctx_commitscalar(ctx)) != 0, cleanup);
    cc_require(ccn_cmp_internal(n, ccsae_ctx_peer_CE_x(ctx), ccsae_ctx_CE_x(ctx)) != 0, cleanup);
    cc_require(ccn_cmp_internal(n, ccsae_ctx_peer_CE_y(ctx), ccsae_ctx_CE_y(ctx)) != 0, cleanup);
